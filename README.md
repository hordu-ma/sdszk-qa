...
| 阶段 1B / G1 | 模拟工程闭环和金标治理工具可用，未通过 G1 | 高中议题式闭环、64 例冒烟回归、140 例内部金标 v0（双评/仲裁全流程）和同镜像稳定环境晋级已实现；真实专家金标评测未完成 |
...
自 2026-07-17 起，上述外部依赖项按主计划 §0.5 自助开发模式冻结为补签清单，不再阻塞开发。自助优先级 1–5 的内部工程增量已完成：检索质量调优与失败取证、内部金标 v0 全流程、引用定位/资料有效期/资料不足策略、诊断规则字典 v2，以及工作台专业编辑、可用性保护与基础无障碍适配。`luyun-int` 当前为 `stage1-workbench-ux-20260719-r1` 并已通过 Virtus 手动验证；`luyun-demo` 继续保留上一稳定镜像 `stage1-diagnostic-rules-20260719-r1`，尚未晋级。本结果只作为 M1-int 内部收口输入，内部产物一律不得表述为客户确认或专家签字。
