...
自 2026-07-17 起，上述外部依赖项按主计划 §0.5 自助开发模式冻结为补签清单，不再阻塞开发。自助优先级 1–4 已完成并部署：检索质量调优与失败取证、内部金标 v0 全流程、引用定位/资料有效期/资料不足策略，以及诊断规则字典 v2（内部版）；当前双环境发布为 `stage1-diagnostic-rules-20260719-r1`。下一步进入优先级 5（工作台体验：专业编辑、可用性走查与无障碍内部版）。内部产物一律不得表述为客户确认或专家签字。
