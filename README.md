...
| 阶段 2 | WP2.1 内部工程已收口 | 专业输入、显式 Memory 保存/复用、可扩展非评分冲突规则和对齐卡单一事实源已进入集成环境；不代表阶段 2 或 G2 完成 |
...
自 2026-07-17 起，上述外部依赖项按主计划 §0.5 自助开发模式冻结为补签清单，不再阻塞开发。自助优先级 1–5 已完成，`stage1-workbench-ux-20260719-r1` 已以同一镜像摘要晋级 `luyun-demo`，完成备份、Tailnet 冒烟和上一镜像回滚恢复；M1-int 内部门结论为通过，详见《[M1-int 收口记录](src/docs/2026-stage1-m1-int-closure.md)》。当前 `luyun-int` 为 WP2.1 收口候选 `stage2-wp21-closure-20260719-r1`，`luyun-demo` 保持 Stage 1 稳定镜像。WP2.1 收口不等于阶段 2 或 G2 完成，内部产物一律不得表述为客户确认或专家签字。
