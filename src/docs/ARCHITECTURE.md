...
> 当前已实现项目/版本、资料/任务、`retrieve_basis` 库内词法检索、最小 ModelClient、Skills 运行时最小集、Memory 最小集、工作台基础页和 `luyun-int`；完整教学设计/诊断、向量混合检索、完整 ModelGateway、多智能体、多模态及 `luyun-demo` 仍属目标能力。
...
- `src/apps/api/services/*`：审计、问答编排、最小 ModelClient、教学项目、知识处理、Skills 运行时（`skill_runtime.py`）和 Memory（`memory_service.py`）；HTTP 路由保持输入/权限/响应编排薄层
- `src/apps/api/schemas/*`：Pydantic 模型（含 Skill 输入输出契约）
...
| 组件 | 职责 | 阶段 | 当前状态 |
| --- | --- | --- | --- |
| SkillDefinition / SkillRun | 版本化任务技能、Schema、配额、审计 | 1 必交最小集 | 已有最小基线：注册表 + 权限/Schema 校验 + 运行留痕；配额/降级策略字段已登记、未启用执行 |
| UserPreference / ClassContextProfile | 显式工作记忆，可删可审计 | 1 最小集 | 已有最小基线：读写、一键清除、导出 |
| MemoryInjectionAudit | 记录注入到某次 SkillRun 的记忆引用 | 1 | 已实现：显式 `memory_refs` + 快照留痕；已删引用拒绝注入 |
| Agent DAG | 只调用 Skills 白名单，不另开写接口 | 4 门禁交付 | 未开始 |

当前唯一达到基线成熟度的 Skill 为 `skill.retrieve_basis`（v1.1.0，pg_trgm 库内词法检索 + 资料不足阈值）。其余阶段 1 Skills 的输入输出 Schema 属阶段 0《产品 Skills 目录 v1》冻结范围，未冻结前不在代码内发明契约。
...
当前已从单一问答 MVP 前进到可部署的阶段 1A 工程骨架：问答兼容链路、Teaching Project/Version、知识审核与 pg_trgm 库内检索、任务审计、最小 ModelClient、Skills 运行时最小集、Memory 最小集和桌面工作台共同运行在 `luyun-int`；后续继续补齐向量混合 RAG、注入确认 UX、纵向样板、完整 ModelGateway 和 `luyun-demo` 晋级。
