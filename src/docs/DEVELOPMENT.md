...
- 当前已实现：vLLM 问答主链、教学项目/资料/任务、版本化语义 RAG、六个样板 Skills、显式 Memory、纵向样板、可配置诊断规则字典 v2（非评分）、教学成果结构化专业编辑、未保存保护、版本差异、结构化 Word 导出、带来源标记的版本化工程评测、专家案例批量导入/双评/仲裁工程门禁，以及 `luyun-int`/`luyun-demo` 同镜像晋级基线。模拟资料、占位案例、工程评测和内部可用性验证不得写成专家验收。详见 [README](../../README.md)。
