...
- 当前已实现：vLLM 问答主链、教学项目/资料/任务、版本化语义 RAG、七个产品 Skills、显式 Memory、纵向样板、可配置诊断规则字典 v2（非评分）、教学成果结构化专业编辑，以及 Stage 2 WP2.1 的专业输入、显式班情/模板保存复用、可扩展非评分冲突规则和对齐卡单一事实源。M1-int 与 WP2.1 已内部工程收口，但模拟资料、内部工程评测和内部可用性验证不得写成专家验收，阶段 2/G2 尚未完成。详见 [README](../../README.md)和 [WP2.1 收口记录](2026-stage2-wp21-closure.md)。
