...
- 当前已实现：vLLM 问答主链、教学项目/资料/任务、版本化语义 RAG、八个产品 Skills、显式 Memory、纵向样板、可配置非评分规则、专业输入、WP2.2 结构化生成，以及 WP2.3 的教案结构校正、证据定位、逐条决定、L4 信号和仅采纳项二次修订；WP2.4 交付了评测回归门禁（发布清单变更检测与内部阈值阻断）与抽检队列/L4 信号维度汇总（双评—分歧—第三人仲裁）；WP2.5 增量 1 交付了安全测试与输入防护（恶意文件真实内容校验、提示注入数据定界与历史角色白名单、跨用户越权与角色边界回归套件），增量 2 交付了试点组织白名单与最小 RBAC（`organizations` 与 `users.organization_id`、试点白名单门禁、`services/rbac.py` 收敛的 reviewer 跨组织隔离、平台 admin 专用组织管理）。M1-int、WP2.1–WP2.4 已内部工程收口，WP2.5 增量 1–2 已交付，但模拟资料、分学段正式量规、内部工程评测和内部可用性验证不得写成专家或客户验收，安全防护为工程缓解措施不代表渗透测试，平台 admin 跨组织可见为有意缺口，阶段 2/G2 尚未完成。详见 [README](../../README.md)、[WP2.4 收口记录](2026-stage2-wp24-closure.md)和 [WP2.5 收口记录](2026-stage2-wp25-closure.md)。
- 产品范围、阶段和验收只看一份文档：[主开发计划](2026-luyun-curriculum-pedagogy-development-plan.md)（v1.0 签字版，当前 v1.4；实施与部署证据见[实施记录附录](2026-implementation-log.md)）。目标能力不等于已实现能力，写文档和注释时不要混淆。
- 当前处于自助开发 + 投标演示优先模式（主计划 §0.5–§0.6）：客户/专家输入冻结为补签清单，优先按内部代理决策、内部金标 v0 和 M0-int/M1-int/M2-int 内部门推进桌面 Web 工程闭环；真实用户试点、专家复核和外部签字后置，内部产物不得表述为客户确认或专家签字。小程序保留接口约束，待阶段 4 核心能力和投标演示基线稳定后作为全计划最后实施包。
...
11. 正式评测集只有在来源审核通过、无 `placeholder` 案例、每例获得两位不同审核人共识或第三人仲裁后才能冻结；复核记录不可覆盖。
