...
| 专家金标 | 治理工具已实现，当前没有真实金标 | `evaluation_case_reviews`、案例金标状态和评测治理工作台 | 批量导入真实案例；两位专家独立复核，分歧由第三人仲裁；移除全部占位标记 | 双专家一致性、仲裁独立性、专业阈值、一票否决项 |
...
- 发布：`stage1-gold-review-20260717-r1`。
- 迁移：`k1f2a3b4c567 (head)`。
...
- 金标批量导入、双评/第三人仲裁、占位案例防冻结和报告接口已部署到双环境；已验证同镜像晋级、`j0 → k1 → j0 → k1` 回滚恢复、双评/仲裁、占位门禁和真实 vLLM SSE。该工程工具状态不代表真实专家金标已经到位。
