...
## 20. WP2.5 第一增量（2026-07-20，安全测试与输入防护）

工程实施记录（2026-07-20，WP2.5 第一增量，主计划 WP2.5 第 1 条）：

（a）恶意文件防护：上传在既有后缀白名单、大小上限之外新增真实内容校验 `validate_upload_content`（不信任客户端声明 MIME）——任何后缀拒绝可执行魔数（PE/ELF/Mach-O/Java class）；`.pdf` 必须以 `%PDF-` 开头；`.docx` 必须是含 `[Content_Types].xml` 与 `word/` 结构的 ZIP，且拒绝 `vbaProject.bin` 宏载荷；`.txt/.md` 必须为 UTF-8 且不含 NUL 字节。不符内容返回 415，错误信息不回显文件内容。

（b）提示注入隔离：系统提示新增「数据与指令边界」规则；用户可控的主题上下文（含自定义会话主题）全部包裹在【主题上下文数据-开始/结束】定界块内并声明块内指令无效；历史消息注入前强制角色白名单（仅 user/assistant），防止任何存量数据提升为 system 指令。

（c）安全回归测试套件：新增 `tests/test_security_hardening.py`（8 例纯函数：五类恶意内容拒绝、注入话术保持在数据块内、历史角色不升权）与 `tests/test_security_boundaries.py`（集成：跨用户访问 11 个端点全部 403/404、教师角色对审核/复核/抽检/全局信号 6 个端点 403、匿名 401/403、恶意文件 API 面 415、登录与 `/auth/me` 响应不含口令散列）。ruff、basedpyright、74 例非集成测试、10 例集成测试（临时 pgvector PostgreSQL 17）全部通过；本轮无 Schema 变化、无前端改动。

部署验收记录（2026-07-20，WP2.5 第一增量）：`luyun-int` 部署 `stage2-wp25-security-20260720-r1`，API/Web 镜像 ID 为 `95b5bc8a8151...` / `1bd060766861...`，数据库保持 `n3b4c5d6e789 (head)`。真实链路验证：五类恶意文件（假 DOCX/PE、假 PDF/脚本、含 NUL 文本、宏 DOCX、EXE）上传均 415 且正常 Markdown 202；跨用户访问四类资源 403/404、教师角色三类复核入口 403、匿名 401/403；注入话术经自定义主题与用户消息走真实 vLLM SSE，链路稳定并以 `[DONE]` 结束；`/api/workbench/model-status` 未降级；登录与 `/auth/me` 不泄露口令散列。应用实际回滚至 `stage2-wp24-spotcheck-20260720-r1` 后健康并恢复当前镜像，无数据库变更。发布前 PostgreSQL 归档（397 TOC 条目，容器内 pg_dump）与 env 快照位于 `/home/pgx/backups/luyun-sizheng/20260720-stage2-wp25-security-r1-predeploy/`，归档 SHA256 为 `afc6c8d1c4ef98c3715b2ce779e57788256a2fcd738780cc17a0d3507fc6e87a`。临时入侵者账号、冒烟项目、会话与自定义主题已清理；`luyun-demo` 继续保持 `stage1-workbench-ux-20260719-r1`。

本记录完成 WP2.5 第一增量（安全测试与输入防护）。试点组织白名单与最小 RBAC/跨组织隔离、模型/RAG/队列分级降级、备份恢复与任务恢复演练、试点并发指标为后续增量；提示注入防护为工程缓解措施，模型对注入话术的语义抵抗仍属内部工程口径，不代表安全审计或渗透测试结论。不改变阶段 2、G2 未完成的结论。

## 21. WP2.5 第二增量（2026-07-22，试点组织白名单与最小 RBAC）

工程实施记录（2026-07-22，WP2.5 第二增量，主计划 WP2.5 第 2 条）：

（a）组织模型与白名单：新增 `organizations`（code 唯一、status=pilot_active/suspended）与 `users.organization_id`（nullable FK，ON DELETE SET NULL）；白名单 = pilot_active。新增 `get_pilot_user` 依赖作用于整个工作台路由——平台 admin 放行，其余用户须属白名单组织否则 403。

（b）最小 RBAC 收敛与跨组织隔离：把分散的 `role not in {admin, reviewer}` 判定收敛到 `services/rbac.py`（`is_platform_admin`/`is_privileged`/`owner_in_actor_scope`/`scope_owner_ids`/`require_pilot_membership`）。reviewer 只能访问本组织资源、平台 admin 覆盖全部，覆盖全部特权跨用户路径：评测 `get_accessible_dataset`/`review_dataset`/`list_review_queue`/`submit_case_review`/`list_case_reviews`、抽检四条路径、`l4_signal_summary` 项目与全局两支、资料审核 `review_document`。平台 admin 组织无关且独占白名单开关为显式缺口（见《WP2.5 收口记录》§5.2）。

（c）组织管理：新增 `/api/organizations`（平台 admin 专用，不受试点门禁限制）列出/创建/置状态/指派用户组织。迁移 `o4c5d6e7f890` 创建组织表与默认试点组织 `luyun-pilot-default` 并回填存量用户；`seed_demo` 与测试夹具共用同一默认组织口径。

（d）验证：新增集成用例 `test_cross_org_isolation_and_pilot_whitelist`（暂停组织成员 403、org A reviewer 对 org B 项目/数据集/报告/审核/资料审核/L4 全部 403/404、复核队列不含跨组织数据集）。ruff、basedpyright、74 例非集成测试、11 例集成测试（临时 pgvector PostgreSQL 17）通过；独立测试库迁移 `n3 → o4 → n3 → o4` 往返并验证默认组织创建与用户回填。

部署验收记录（2026-07-22，WP2.5 第二增量）：`luyun-int` 部署 `stage2-wp25-orgrbac-20260722-r1`，API/Web 镜像 ID 为 `5ed2cba33a88...` / `b2e874eb6a72...`，数据库升级至 `o4c5d6e7f890 (head)`（新增 `organizations` 表 + `users.organization_id` 列，nullable+回填）。默认组织创建、4 个存量用户回填。真实链路验证：存量 demo_teacher 工作台可用；平台 admin 组织管理成功、教师越权 403；暂停组织成员登录成功但工作台 403；org A reviewer 对 org B 资源全部 403/404 且复核队列不含跨组织数据集；平台 admin 跨组织可见（记录缺口）；vLLM 未降级。应用实际回滚至 `stage2-wp25-security-20260720-r1` 后健康（nullable+回填保证向后兼容，无需数据库降级）并恢复当前镜像，本机与 Tailnet 两个 HTTPS `/healthz` 通过。发布前 PostgreSQL 归档（397 TOC 条目，容器内 pg_dump）与 env 快照位于 `/home/pgx/backups/luyun-sizheng/20260722-stage2-wp25-orgrbac-r1-predeploy/`，归档 SHA256 为 `bfb6d811c1262f7c64031308e1956dfc173075c4e154c590b69d9363a6ee6d74`。临时组织、账号、项目与审计已清理；`luyun-demo` 继续保持 `stage1-workbench-ux-20260719-r1`。

本记录完成 WP2.5 第二增量（试点组织白名单与最小 RBAC）。模型/RAG/队列分级降级、备份/任务/版本回滚演练、试点并发指标为 WP2.5 剩余增量；平台 admin 跨组织可见为有意缺口，多校真实试点前须重新界定。不改变阶段 2、G2 未完成的结论。
