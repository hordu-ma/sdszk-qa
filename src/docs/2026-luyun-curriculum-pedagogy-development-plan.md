...
> - 版本：v1.3（在 v1.2 基础上新增投标演示优先执行轨道与小程序后置实施顺序；合同范围、阶段归属、G0–G4 验收门槛与禁止项不变）
> - 日期：2026-07-19（v1.0 签字版日期 2026-07-15；v1.1 平实化修订 2026-07-16；v1.2 自助开发模式 2026-07-17）
> - 状态：签字版 + 自助开发/投标演示优先模式生效；客户与专家暂不参与期间，外部签字项按 §0.5 冻结为补签清单，工程按内部代理决策先行开发，G0/G1 外部签字仍为最终验收前提
...
- 2026-07-15 签字前审计：小程序确认为合同硬交付，合同归属阶段 3 的轻量端；补入合规适用性、生成内容标识、四库评测隔离、独立红队、Data/System Card、持续评测与 RTO/RPO。
...
13. 小程序为合同硬交付，归属阶段 3、但实施顺序后置：先完成阶段 1–4 的桌面 Web 核心闭环、增强能力和投标演示基线，再在全计划最后建设轻量端；轻量端完成统一登录、可信轻问答、收藏/历史、通知和异步任务进度，并可跳转网页继续复杂编辑；完整备课工作台、Memory 全量管理和后台审核不进小程序。
...
17. 投标演示优先执行轨道（2026-07-19 起）：在不改变合同范围和 G0–G4 门槛的前提下，先完成可部署、可回滚、可重复演示的桌面 Web 主链与阶段 0–4 工程能力；真实用户试点、专家复核、客户模板、平台接口和外部签字不再阻塞内部工程，统一记为补签/替换清单。
...
- 已确认：小程序属于合同硬交付，合同归属阶段 3，边界为轻量访问和跨端续办，不承载完整桌面工作台；当前执行顺序将实际实现后置到全计划最后。
...
1. **内部代理替代外部输入。** 对尚未获得的用户资料、专家规则、客户 Word 模板、平台接口和试点反馈，使用明确标记的 `synthetic` / `internal_authored` 数据、内部标准模板和代理决策继续开发；所有替代项进入补签/替换台账。
2. **先做可演示工程闭环。** 优先完成桌面 Web 的真实 API、版本、审计、降级、导出、演示脚本和双环境部署；演示只调用已经实现的 Skills，不用静态原型冒充后端能力。
3. **用户参与改为后置验证。** 真实教师试用、专家双评、客户模板确认、平台统一身份和试点反馈不再作为当前开发前置条件；恢复参与后按替换台账逐项复核，不得把内部结果改名为外部结论。
4. **按演示风险而非用户等待排序。** 先处理会导致演示中断、结果不可追溯、无法回滚或无法解释的工程风险，再扩展能力面；每个纵向增量仍必须经过 `luyun-int`、运行态验证、回滚检查和文档对齐。
5. **小程序最后实现。** 阶段 3 期间只保留小程序的接口、身份、通知和跨端续办约束，不投入完整实现；待桌面 Web 核心闭环、阶段 4 增强能力和投标演示基线稳定后，作为全计划最后一个实施包完成，再进行统一登录、轻问答、收藏/历史、通知和任务进度验收。

当前执行顺序调整为：

`WP2.3 证据化诊断 → WP2.4 内部评测/仲裁工程化 → WP2.5 安全、性能、恢复 → WP2.6 内部投标演示脚本与稳定基线 → 阶段 3/4 Web 能力和增强门禁 → 小程序后置收口 → 外部补签与 G0–G4 决策门`。

其中“内部投标演示完成”只表示工程链路可运行、可解释、可回滚，不等于真实教师可用性、专家专业确认、客户验收或 G 门通过。
...
- 完整桌面优先教学工作台（当前内部版已支持结构化专业编辑、不可变版本保存、未保存保护和基础无障碍操作；字段级局部重生成、富文本编辑及真实教师可用性研究仍未完成）。
- 管理端、审核流、资源库、阶段 3 合同归属的小程序轻量端和区域运营看板；小程序实际实现后置。
...
因此，本计划把轻量诊断、局部修改、版本对比、标准导出、首批产品 Skills 与 Memory 最小集前置到阶段 1–2，再在阶段 3–4 扩展四学段、一体化和增强能力，最后实现小程序轻量端。
...
- 多人实时协同编辑。
- 完整小程序工作台（小程序为阶段 3 合同硬交付但实际实现后置，只交付轻量端；复杂编辑、完整 Memory 管理、审核和管理看板由网页端承担）。
- 自动将教师成果发布到公共资源库。
- 生产环境外部模型兜底。
- 大规模通用知识图谱建设。
...
- 阶段 3 的 8–12 周先完成四学段、一体化、审核、组织和运营 Web 能力，并只冻结小程序接口与跨端协议；真实试点与平台接口按补签清单后置。
- 阶段 4 的 16–22 周先完成多智能体、多模态、微调评估和投标演示 v4；小程序轻量端在阶段 4 核心能力稳定后作为全计划最后实施包，条件不满足时不得用压缩验证替代延期。
...
11. 小程序确认为阶段 3 合同硬交付，保持轻量端边界；除接口/协议冻结外，实际实现后置到阶段 4 核心能力和投标演示基线之后。
...
18. v1.3：新增投标演示优先执行轨道；客户/专家暂不参与期间以内部代理决策和内部数据推进桌面 Web 工程闭环，真实用户/专家验证后置；小程序保留阶段 3 合同归属但将实际实现移动到全计划最后，不改变合同范围和 G0–G4 门槛。
...
- v1.3：新增 §0.6 投标演示优先执行轨道，调整阶段 3/4 的执行顺序与周期假设，新增阶段 4 最后实施的小程序收口包；同步 README、DEVELOPMENT 和 ARCHITECTURE，明确不改变合同范围、阶段归属和 G0–G4 外部验收门槛。
