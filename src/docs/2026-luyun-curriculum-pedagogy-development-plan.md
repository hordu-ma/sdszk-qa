...
截至 2026-07-15，当前项目已从思政场景问答 MVP 前进到“问答兼容链路 + 阶段 1A 首个可部署增量”，已经具备：
...
上述状态是实施快照，不改变本计划的范围、工作包和 G1 门槛。当前结论为：**阶段 1A 进行中，首个可部署增量完成；阶段 1A 整体、阶段 1B 和 G1 均未完成。**

### 1.2 当前尚未具备的关键能力

- 统一身份、组织同步和完整角色权限（目标由思政课平台用户管理提供注册/认证分级，本仓对接消费；见 §2.6）。
- 课程标准、教材、政策和地方资源的完整可信 RAG（当前仅有审核门禁和可追溯词项检索基线，尚无向量 + Reranker 混合检索）。
...
- 专家评测集、发布回归门禁和教师采用反馈。
- 完整桌面优先教学工作台（当前已有项目/版本/资料/任务/检索基础页，生成—诊断—导出闭环未完成）。
...
- vLLM/Ollama 完整 Provider 契约、标准模型资产清单和 `base-spark` 集成/稳定演示双环境晋级机制（当前只有最小 ModelClient 与 Ollama 过渡 Provider）。
...
当前项目 API 已通过最小 ModelClient 使用逻辑模型名、Provider 标识和 Provider 模型 ID，并支持 Ollama 原生流式接口或 OpenAI 兼容接口。Provider 能力登记、认证隔离、任务路由和一致性回归仍未完成；不能把“能返回文本”视为模型服务已经统一。该差距仍须通过完整 ModelGateway 和 Provider Adapter 解决。
