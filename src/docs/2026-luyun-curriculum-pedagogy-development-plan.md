...
> - 版本：v1.0（签字版）
> - 日期：2026-07-15
> - 状态：签字版；经客户与项目责任人签署后进入阶段 0，全面业务开发仍须通过 G0
...
13. **小程序为合同硬交付：** 阶段 3 建设轻量端，必须完成统一登录、可信轻问答、收藏/历史、通知和异步任务进度，并可跳转网页继续复杂编辑；不得将完整备课工作台、Memory 全量管理或后台审核塞入小程序。
14. **合规与安全是发布门：** 阶段 0 完成适用性矩阵、数据分类、生成内容标识、投诉纠错和备案/安全评估判断；阶段 1 起按 OWASP 风险面执行独立红队，不以普通功能测试替代。
15. **评测四库隔离：** 开发集、CI 回归集、密封验收集和线上事件集分权管理；训练、调参和开发人员不得接触密封验收集。
...
`2026-luyun-external-solution-v1.0.md` 及对应 Word 是本文的对外表达版，不新增或变更范围；如表述冲突，以本文签字版为准。
...
- 多人实时协同编辑。
- 完整小程序工作台（小程序为阶段 3 硬交付，但只交付轻量端；复杂编辑、完整 Memory 管理、审核和管理看板继续由网页端承担）。
- 自动将教师成果发布到公共资源库。
- 生产环境外部模型兜底。
- 大规模通用知识图谱建设。
...
小程序为合同硬交付，安排在阶段 3 建设，不与阶段 1–2 的 Skills/Memory 核心闭环并行抢资源。其功能边界固定为：
...
小程序不承载大文档复杂编辑、完整 Memory 管理、资源审核、管理看板和用户自建开放工具；这些能力继续由网页端承担。
...
阶段 4 是合同硬交付阶段，建议周期 14–20 周，以 G4 作为合同最终验收门。多智能体、多模态与微调均采用**能力门禁式交付**：必须形成规格、可运行样板、评测、降级与决策记录；不承诺任何未过门槛的能力进入生产。Agent 只能编排产品 Skills，不得绕过 Skill 注册表。
...
合同覆盖阶段 0–4、正式/验收默认 vLLM、Ollama 过渡/备用、按节点双环境持续部署、**产品 Skills 与核心用户 Memory 纳入计划**、**注册用户/认证用户两级制及平台侧实现**、**阶段 3 小程序轻量端硬交付**均已确认方向，不再作为“做不做”的待决项。仍需确认：
