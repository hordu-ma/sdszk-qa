...
状态（2026-07-20）：第一增量（安全测试与输入防护）已完成并部署 `luyun-int`（`stage2-wp25-security-20260720-r1`，含回滚演练）——上传真实内容校验（可执行魔数/PDF 头/DOCX 结构与宏载荷/UTF-8 文本，415 拒绝）、提示注入数据定界与历史角色白名单、跨用户越权与角色边界安全回归套件（跨用户 403/404、教师复核入口 403、匿名 401/403、响应不泄露口令散列）。试点组织白名单与最小 RBAC/跨组织隔离、模型/RAG/队列分级降级、备份恢复与任务恢复演练、试点并发指标为后续增量。提示注入防护为工程缓解措施，不代表安全审计或渗透测试结论。详见《WP2.5 收口记录》与《实施记录附录》§20。
