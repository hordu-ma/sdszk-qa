...
> - 版本：v0.5（用户注册/认证分级 + 平台用户管理边界）
> - 日期：2026-07-15
> - 状态：开发前讨论稿，v1.0 签字前不进入本仓业务功能开发；允许第 0.4 节列出的零业务污染探针
> - 适用范围：鲁韵思政大模型的教学设计生产、教学设计诊断、产品 Skills、核心用户 Memory、受控多智能体、多模态、模型专项优化及投标演示部署；**用户注册与身份认证能力规范见 §2.6，实现落点不在本仓库**
> - 文档权威：本合同范围、阶段、验收与工程顺序以本文为唯一主依据；其余产品/功能文档必须与本文对齐，冲突时以本文为准
...
- 2026-07-15 工程评审：纳入产品 Skills 与核心用户 Memory；以纵向样板优先、阶段 4 能力门禁化、文档驱动开发等优化原则修订排期口径。
...
> 可靠教学成果 = 权威课程知识 + 课程教学结构模型 + 专家规则 + 受控产品 Skills（工作流） + 核心用户受控 Memory + 经专项评测的本地模型 + 教师与专家反馈闭环
...
4. 教学诊断默认不输出总分和教师排名，必须基于原文证据、专家规则和课程依据提出形成性建议；仓库内不建设教师/学生评分排名模块。
5. 合同范围覆盖阶段 0–4，以 G4 为最终验收门。阶段 4 必须交付受控多智能体、多模态与微调评估闭环；多智能体、多模态与微调均采用**能力门禁式交付**（可运行样板 + 评测 + 降级/No-Go），不得为形式交付强行上线不合格能力。
6. 正式环境和最终验收默认采用 vLLM 推理服务；Ollama 仅作为前期开发、兼容性验证和明确标注的备用 Provider，业务代码不得绑定任一推理引擎。
7. `base-spark` 采用集成环境与稳定演示环境双轨部署；每个可验收开发节点必须先完成部署、`virtus` 端到端验证和回滚检查，再晋级稳定演示版本。演示轨道不得抢跑尚未完成的教学专业能力，不得用原型冒充已实现后端。
8. **产品 Skills**：教师任务以版本化、有 Schema、有权限与审计的受控技能暴露（检索依据、生成蓝图、诊断、导出等），而不是开放插件市场或自由 Agent 工具箱；阶段 4 Agent 必须复用同一批产品 Skills。
...
12. **用户身份两级制（平台侧实现）**：完成手机号验证与姓名、工作单位自填后为**注册用户**；完成第 5 步权威源/组织侧升级后为**认证用户**。默认不采集未核验身份证号。注册/认证能力在**思政课平台用户管理**开发，**本仓库（sdszk-qa）不实现注册与 KYC**，仅消费平台下发的身份声明与会话。
...
- 已确认：产品 Skills 与核心用户 Memory 纳入阶段 0–4 开发计划（见第 2.5 节）；Memory 形态为工作记忆而非通用用户画像。
...
- 待确认：思政课平台用户管理的具体系统名、接口契约、token/claims 字段与本仓对接方式（见 §2.6.5）。
- 待确认：现有平台的统一身份、组织、资源和审计接口（与上项合并推进）。
...
- 基于专家规则和原文证据的教学设计诊断。
- **产品 Skills 注册表、输入输出 Schema、权限、调用审计与任务入口。**
- **核心用户 Memory（偏好、班情档案、模板钉选、注入确认与删除）。**
- 异步任务、Worker、共享缓存、失败重试和高峰降级。
- 模型、提示词、规则、知识库和成果版本的统一追溯。
- 专家评测集、发布回归门禁和教师采用反馈。
- 桌面优先的教学工作台 UI（当前前端以 Vant 轻交互问答为主）。
- 管理端、审核流、资源库、（可选）小程序和区域运营看板。
...
- 薄路由厚服务的业务分层（核心编排仍集中在 `routes/chat.py`）。
...
- 缺少产品 Skills 与 Memory 的显式设计时，系统会退回“长会话 + 隐式上下文”，无法审计、无法复用、难以验收。

因此，本计划将轻量诊断、局部修改、版本对比、标准导出、**首批产品 Skills 与 Memory 最小集**前置到阶段 1–2，再在阶段 3–4 扩展四学段、一体化和增强能力。
...
4. **先服务层后功能：** 阶段 1 启动时优先把 LLM 调用与编排移出路由，避免在 `chat.py` 上继续堆叠。
5. **演示不抢跑：** D0–D3 只保证可访问与真实已实现能力；未完成能力必须标记为原型/预置。
6. **桌面优先工作台：** 备课/诊断/版本/导出以桌面网页为主；小程序阶段 3 按数据决策，不与核心闭环捆绑。
...
首页按教师任务组织，而不是按 AI 技术名词组织。每个入口对应一个或多个**产品 Skill**（见 2.5）：
...
6. 在阶段 4，在门禁通过的前提下，用受控多智能体编排**复用上述 Skills** 完成复杂任务，并把图片、扫描件、音频和短视频作为可定位、可纠正的教学证据。
...
- 多人实时协同编辑。
- 完整小程序工作台（小程序非阶段 1–2 必交；阶段 3 数据决策）。
- 自动将教师成果发布到公共资源库。
- 生产环境外部模型兜底。
- 大规模通用知识图谱建设。
...
**定义：** 产品 Skill 是面向教师任务的、版本化的受控工作流单元，具有稳定的 `skill_id`、输入 Schema、输出 Schema、权限、配额、审计和失败降级策略。它不是提示词别名，也不是开放 Agent 工具市场。
...
对接完成前：本仓文档与演示可继续使用本地用户；**不得**在本仓扩展自建注册以“临时替代”平台用户管理。
...
8. 受控多智能体层：以 DAG 调用产品 Skills，结构化 Schema、预算和人工门禁完成复杂协作。
9. 多模态证据层：解析图片、扫描件、音频和短视频，并保留区域/页码/时间点与人工纠错。
...
- Pydantic/JSON Schema：约束教学设计、诊断、Skill 输入输出和 Memory 对象，并进行服务端二次校验。
- 前端以桌面优先教学工作台为目标形态；可保留 Vue 3，但阶段 1 起备课/诊断主路径不得以手机组件库交互为唯一设计。
...
| 阶段 1：可信底座与纵向样板 | 6–8 周 | 建立共用底座、Skills 运行时、Memory 最小集，并打通一个学段/课型纵向样板 | 内部 Alpha、RAG、任务中心、Skills、Memory、样板闭环 | G1：样板是否值得扩展 |
| 阶段 2：生产与诊断闭环 | 8–10 周 | 在样板范围内完成可编辑生成、证据化诊断、导出、Memory 复用与试点 | Beta、样板教师完整闭环、试点报告 | G2：样板范围是否达到受控开放条件 |
| 阶段 3：一体化特色与生产运营 | 8–12 周 | 扩展四学段、一体化、审核、组织和运营；小程序按数据决策 | 生产候选、中心特色能力和运营制度 | G3：是否扩大区域开放 |
| 阶段 4：智能协作与多模态增强 | 14–18 周 | 交付门禁化多智能体（复用 Skills）、多模态证据与微调评估闭环 | 合同最终候选 v4.0、增强能力或正式 No-Go/缩范围包、综合试点及最终验收 | G4：是否完成合同最终验收 |
...
- 单独确认小程序和多人实时协同是否属于合同，不与已经确认的多智能体、多模态、Skills/Memory 范围混淆。
...
- 原型交互按**桌面优先工作台**设计；可附移动浏览说明，但不把完整备课交互绑在小程序上。
- 用真实教师完成可用性走查。
...
- 模型、提示词、规则、知识库、Schema、**Skill 版本**和导出模板版本记录。
...
- 注册并打通 2.5.1 阶段 1 Skills（含 `skill.manage_memory`）。
...
- 阶段 1 Skills 均可完成至少一次端到端 SkillRun，并留下完整审计。
...
- 采纳/忽略写入 L4 信号；默认不因采纳结果给教师打分或排名。
...
根据网页端真实使用数据决定是否建设小程序。小程序**不是**阶段 1–2 必交，也不与 Skills/Memory 核心闭环捆绑。若建设，首期只承担：
...
阶段 4 是合同硬交付阶段，建议周期 14–18 周，以 G4 作为合同最终验收门。多智能体、多模态与微调均采用**能力门禁式交付**：必须形成规格、可运行样板、评测、降级与决策记录；不承诺任何未过门槛的能力进入生产。Agent 只能编排产品 Skills，不得绕过 Skill 注册表。
...
每条流程可独立门禁：未通过时允许路径 B——保留阶段 3 单工作流、交付正式差距报告与复现材料，不得为验收强行打开不合格开关。
...
| 评分/排名模块回流 | 非范围清单；废弃 extend-scoring 可执行路径 | 删除或关闭相关接口与报表 |
...
- `.github/skills`：工程 playbook；`extend-scoring` 保持废弃/占位，不得引导实现评分模块。
...
4. 多智能体、多模态和微调统一为阶段 4 **门禁式**交付；均保留路径 A/B（上线或正式 No-Go/缩范围）。
...
合同覆盖阶段 0–4、正式/验收默认 vLLM、Ollama 过渡/备用、按节点双环境持续部署、**产品 Skills 与核心用户 Memory 纳入计划**、**注册用户/认证用户两级制及平台侧实现**均已确认方向，不再作为“做不做”的待决项。仍需确认：
...
11. 阶段 4 三条受控多智能体流程、Agent 角色、**Skills 白名单**和人工确认点。
...
21. 小程序是否为合同硬范围；建议继续采用阶段 3 数据驱动决策，不与多智能体、多模态、Skills/Memory 混为一项。
22. 是否需要单一诊断分数；**计划默认全周期不提供**，建议签字确认。
23. 41–55 周周期、阶段 4 的 11–14 人高峰团队和客户侧配合资源；若实际编制更小，确认“最小可行合同切片”是否收缩阶段 3/4 并行度。
...
4. 再确认第 8–13 节：41–55 周合同周期、阶段 0–4、增强门禁、微调双路径和 G4 最终验收。
