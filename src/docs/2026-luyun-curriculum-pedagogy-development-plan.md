...
> - 版本：v0.3（模型服务与持续部署口径修订稿，待确认）
> - 日期：2026-07-15
> - 状态：开发前讨论稿，未批准前不进入代码开发
> - 适用范围：鲁韵思政大模型的教学设计生产、教学设计诊断、受控多智能体、多模态、模型专项优化及投标演示部署

## 0. 文档目的与结论

本计划基于以下材料形成：

- 当前问答 MVP 的代码、架构、测试和部署文档。
- `2026-product-extension-development-suggestions.md`。
- `2026-product-extension-feature-spec.md`。
- `鲁韵思政大模型建设方案_详细版_最终版.docx`。
- 上次客户汇报后的核心反馈：产品必须体现“教学与课程论”体系，尤其要提升教学设计生产和教学设计诊断的专业可信度。

客户对“单纯部署开源大模型难以胜任专业教学设计与诊断”的顾虑成立。本项目不能继续采用“开源模型 + 一段长提示词 + 直接输出完整教案”的建设方式。

本计划采用的能力公式是：

> 可靠教学成果 = 权威课程知识 + 课程教学结构模型 + 专家规则 + 受控工作流 + 经专项评测的本地模型 + 教师与专家反馈闭环

核心产品目标是：

> 教师能够在可信依据支持下，更快形成一份可解释、可局部修改、可诊断、可导出、可持续迭代的教学成果；中心能够管理专业标准、资料、审核和质量，而不是只管理模型和问答次数。

### 0.1 本计划的七项核心决策

1. 产品主对象从“聊天会话”升级为“教学成果项目”。
2. 教学专业底座采用“课程层—单元层—课时层—实施反思层”四层对象模型。
3. 教学设计先生成目标、证据、任务和评价的对齐结构，再生成自然语言正文。
4. 教学诊断默认不输出总分和教师排名，必须基于原文证据、专家规则和课程依据提出形成性建议。
5. 合同范围覆盖阶段 0–4，以 G4 为最终验收门；阶段 4 必须交付受控多智能体、多模态和微调评估闭环。微调模型是否进入生产由质量、安全、数据和硬件门槛决定，不能为满足形式交付而强行上线。
6. 正式环境和最终验收默认采用 vLLM 推理服务；Ollama 仅作为前期开发、兼容性验证和明确标注的备用 Provider，业务代码不得绑定任一推理引擎。
7. `base-spark` 采用集成环境与稳定演示环境双轨部署；每个可验收开发节点必须先完成部署、`virtus` 端到端验证和回滚检查，再晋级稳定演示版本。

### 0.2 假设/待确认

- 假设：合同全周期主要用户仍是一线思政课教师、教研员和中心专家，不直接面向学生开放生成式对话。
- 已确认：合同建设范围覆盖阶段 0–4，包括受控多智能体、多模态和可能的微调，以 G4 作为合同最终验收门。
- 已确认：投标演示环境部署在 `base-spark`，由同一 Tailnet 内的 `virtus` 浏览器访问。
- 已确认：正式环境和最终验收默认采用 vLLM；`base-spark` 当前 Ollama 仅作为过渡开发和备用 Provider，需在 D0 完成 GB10/arm64 上的 vLLM 专项验证。
- 已确认：开发过程中按节点持续部署，`base-spark` 保留高频更新的集成环境和可随时演示的稳定环境，未经门禁不得用开发版本覆盖稳定演示版本。
- 假设：最终生产环境继续坚持校内部署，本地模型不得静默切换到外部服务；`base-spark` 仅作为投标演示和研发验证环境，不替代客户生产验收环境。
- 待确认：山东师范大学信息中心可用 GPU、显存、CPU、内存、存储、备份和网络条件。
- 待确认：现有平台的统一身份、组织、资源和审计接口。
...
当前项目仍是思政场景问答 MVP，已经具备：

- 本地账号登录、JWT 鉴权。
- 固定主题与自定义主题。
- 会话创建、历史会话和消息留存。
- 基于 vLLM OpenAI 兼容接口的 SSE 流式问答。
...
- 统一身份、组织同步和完整角色权限。
- 课程标准、教材、政策和地方资源的可信 RAG。
- 来源引用、引用定位、资料有效期和资料不足处理。
- 课程层、单元层、课时层的结构化教学成果。
- 教学设计编辑、局部重生成、版本对比和 Word 导出。
- 基于专家规则和原文证据的教学设计诊断。
- 异步任务、Worker、共享缓存、失败重试和高峰降级。
- 模型、提示词、规则、知识库和成果版本的统一追溯。
- 专家评测集、发布回归门禁和教师采用反馈。
- 管理端、审核流、资源库、小程序和区域运营看板。
- 受控多智能体任务状态、工具权限、预算和单工作流回退。
- 图片/音频/短视频的多模态证据定位、纠错、授权和删除。
- 微调数据流水线、冻结测试集及 Go/No-Go 验收。
- base-spark 专用演示 Compose、Tailscale Serve、Virtus 跨设备验收和演示降级包。
- vLLM/Ollama Provider 统一契约、标准模型资产清单和 `base-spark` 集成/稳定演示双环境晋级机制。

### 1.3 现有方案需要修正的方向

现有最终方案提出十二类模块，方向完整，但如果直接作为一期范围，会产生以下问题：

- “已实现、一期承诺、后续愿景”边界不够清晰。
- 教学设计生成和诊断仍主要以功能描述呈现，缺少统一的课程教学领域模型。
- 验收偏重“能否生成”，缺少专业正确性、证据充分性、教师采用和时间节省指标。
- 教案诊断在最终方案中被放到第二阶段，会导致一期只有“生成”而没有“修改闭环”。
- 多智能体、小程序、研修、资源管理和看板同时出现，容易稀释核心体验。

因此，本计划将轻量诊断、局部修改、版本对比和标准导出前置到阶段 1–2，再在阶段 3–4 扩展四学段、一体化和增强能力。
...
> 首先帮助教师完成“查可信依据、形成可改设计、诊断已有教案、生成课堂成果”四项核心任务，并将专家认可、权属清晰的成果沉淀为中心资源。
...
首页按教师任务组织，而不是按 AI 技术名词组织：

1. 查政策、课标和教材依据。
2. 备一节课或设计一个单元。
3. 诊断并修改已有教案。
4. 查看并使用首个经专家预制的一体化主题样板，参与结构验证；在阶段 3 前不承诺任意主题都可自动生成。
5. 查看、继续编辑和导出我的教学成果。
6. 在阶段 4 使用受控多智能体完成复杂教研任务，并把图片、扫描件、音频和短视频作为可定位、可纠正的教学证据。

### 2.4 全周期边界与分阶段限制

- 面向学生的开放式问答和自动辅导。
- 对教师或学生进行自动总分、排名和思想状态推断。
...
- 生产环境外部模型兜底。
- 大规模通用知识图谱建设。
- 对微调模型“必须上线”的结果承诺；合同必须完成微调准备、实验和 Go/No-Go 决策闭环，但生产采纳以质量和安全门槛为条件。

阶段 0–3 只预留 Agent、多模态资产和训练数据血缘所需的结构与接口，不提前承诺增强能力可用；受控多智能体、多模态教学证据服务和微调决策包在阶段 4 交付。

## 3. 教学与课程论专业框架

### 3.1 专业依据与产品推导边界

官方课程文件为产品提供课程性质、核心素养、课程目标、课程内容、学业质量、教学实施和评价原则。义务教育新课标进一步通过“内容要求、学业要求、教学提示、学业质量”增强教学可操作性；高中思想政治课标强调活动型学科课程、议题、辨析、情境和社会实践。

在这些正式依据上，本项目作出以下产品设计推导：
...
上述产品方法不是国家课程标准规定的唯一教学模式，必须在界面和文档中明确标注为“鲁韵产品方法”，并允许中心专家配置、修订和停用。
...
教师进入产品时，应先选择正在处理“课程、单元还是课时”任务。系统诊断已有材料时也必须先识别材料层级，避免用课时规则诊断单元方案。
...
2019 年学校思想政治理论课教师座谈会提出的“八个相统一”，是专家规则的重要政策来源。产品需要由分学段专家将其转化为可观察的设计证据，但不能由模型机械打标签、自动评分，也不能把八项都设成每一节课的同质化硬指标。规则重点检查政治性与学理性、价值性与知识性、理论性与实践性、教师主导与学生主体等关系是否在具体教学设计中得到恰当体现。
...
5. 受控工作流层：把复杂任务拆成检索、规划、生成、检查、修复、确认和导出步骤。
6. 受控多智能体层：以 DAG、结构化 Schema、工具白名单、预算和人工门禁完成复杂协作。
7. 多模态证据层：解析图片、扫描件、音频和短视频，并保留区域/页码/时间点与人工纠错。
8. 模型网关与优化层：按任务评测并路由本地模型，管理 Provider、模型版本、微调实验和回滚。
9. 评测治理层：离线金标、专家抽检、线上反馈、发布门禁、隐私删除、回滚和安全治理。
...
- vLLM/Ollama 或经评测的本地推理服务：由模型网关和 Provider Adapter 访问，不再由业务路由写死模型地址。
- OCR、ASR、VLM 与媒体处理 Worker：阶段 4 按模态和硬件评测选型，不假设一个文本模型承担全部任务。
- 数据集与模型注册：保存训练/验证/冻结测试版本、适配器/权重、模型卡、许可证、评测和部署状态。
- Pydantic/JSON Schema：约束教学设计和诊断结构，并进行服务端二次校验。
...
教师不需要选择模型。即使早期阶段只有一个合格生成模型，应用也必须通过网关调用，以便后续替换和回滚。
...
- 同一逻辑模型在 vLLM 与 Ollama 上必须分别登记真实模型 ID、量化方式和 Template，并通过 Provider 契约、专业质量、结构化输出、SSE 和性能一致性测试，不能假设“同一模型名称”就代表行为一致。
...
若条件满足，阶段 4 至少完成一个明确目标任务的 LoRA/SFT 实验、盲测、回归、安全和容量比较，并交付模型卡、适配器/权重、训练配置、数据版本及回滚包。若条件不满足或实验无可靠收益，则交付正式 No-Go 报告、可复现基线、数据缺口和下次触发条件；该路径同样视为完成合同中的微调评估闭环，但不得上线不合格模型。
...
| `virtus` | 与 `base-spark` 处于同一 Tailnet，当前在线且 Active | 可作为现场浏览器和投屏终端，但仍需真实浏览器端到端验收 |
| Tailscale Serve | 当前无 Serve 配置 | 需要启用 Tailnet HTTPS、配置 ACL/Grant 并建立持久化 Serve 规则 |
...
| 双环境基础 | 当前尚无 `luyun-int` 集成环境和 `luyun-demo` 稳定演示环境 | 需要建立独立项目名、数据卷、配置、发布门禁和版本晋级机制；稳定演示环境不得被日常开发直接覆盖 |

当前项目 API 直接调用 OpenAI 兼容的 `/v1/chat/completions`。短期可以分别指向 vLLM 或 Ollama 做兼容性验证，但尚未实现 Provider 能力登记、逻辑模型名、认证隔离、任务路由和一致性回归；不能把“能返回文本”视为模型服务已经统一。该差距必须通过 ModelGateway 和 Provider Adapter 解决。
...
首个可从 `virtus` 访问的技术基线建议安排 2–3 周；达到可用于正式投标彩排、包含故障降级和恢复演练的完整 D0–D3 基线，建议安排 3–5 周。该轨道可在计划批准后与阶段 0/1 并行；它只解决部署、接入和演示保障，不会凭空提前尚未开发的教学能力。
...
节点“完成”的定义必须包含：代码和文档完成、自动测试通过、部署到 `luyun-int`、`virtus` 端到端验证、模型/规则/知识/数据版本登记、监控可见和回滚可执行。只有满足这些条件，才可将同一不可变镜像晋级 `luyun-demo`；不得在演示环境现场重新构建。
...
- 稳定演示入口候选为 `127.0.0.1:8088`，集成入口使用另一个经 D0 登记的 loopback 端口；两者均通过环境变量配置。API `8000`、PostgreSQL `5432`、MinIO `9000/9001`、Redis `6379`、Worker、vLLM 和 Ollama 均不得直接暴露给浏览器或 Tailnet。
...
- D-L3：使用只读案例、导出成果和本地录屏完成讲解；若 Tailscale HTTPS 尚未获批，允许通过 Tailscale SSH 端口转发访问 loopback，禁止临时开放公网。
...
- 建议合同完整建设周期：41–55 周，覆盖阶段 0–4；长期运营不计入该周期。
- 阶段 0–3 假设团队为 8–10 人全职核心团队（产品、课程教学、AI、后端、前端、QA），另有共享 DevOps 支持、1–2 名内容运营和四学段兼职专家组。
- 阶段 4 高峰建议扩充到 11–14 人，增加 Agent/LLM 平台、多模态、训练/MLOps、数据质检和隐私安全能力；若不扩充，阶段 4 应按 18–24 周、合同总周期按 45–61 周估算。
...
- 41–55 周按阶段 0–3 的 27–37 周加阶段 4 的 14–18 周估算；投标演示先行轨道与阶段 0/1 并行，原则上不额外增加总周期。
- 阶段 3 的 8–12 周包含连续 4 周四学段试点，前提是 WP3.1–WP3.5 可并行、专家和平台接口按期到位；否则试点及整改需另增加约 4–6 周。
- 阶段 4 的 14–18 周包含连续 4 周增强能力试点，前提是 Agent、多模态、数据和硬件准备已按阶段 0–3 前移；条件不满足时不得用压缩验证替代延期。

### 8.2 总体阶段表

| 阶段 | 建议周期 | 目标 | 可独立交付结果 | 决策门 |
| --- | ---: | --- | --- | --- |
| 阶段 0：联合定义与基线 | 5–7 周 | 冻结专业框架、范围、资料、评测和硬件基线 | 领域模型、量规、种子评测集、原型、合同全周期范围 | G0：是否具备全面开发条件 |
| 阶段 1：可信底座与纵向样板 | 6–8 周 | 建立共用底座并打通一个学段/课型的四层纵向样板 | 内部 Alpha、RAG、任务中心、样板闭环 | G1：样板是否值得扩展 |
| 阶段 2：生产与诊断闭环 | 8–10 周 | 在样板范围内完成可编辑生成、证据化诊断、导出与试点 | Beta、样板教师完整闭环、试点报告 | G2：样板范围是否达到受控开放条件 |
| 阶段 3：一体化特色与生产运营 | 8–12 周 | 扩展四学段、一体化、审核、组织和运营 | 生产候选、中心特色能力和运营制度 | G3：是否扩大区域开放 |
| 阶段 4：智能协作与多模态增强 | 14–18 周 | 交付受控多智能体、多模态教学证据服务和微调评估闭环 | 合同最终候选 v4.0、增强能力、综合试点及最终验收包 | G4：是否完成合同最终验收 |
...
为保证可执行性，阶段 0 分成两个连续子阶段：0A（约 3 周）完成范围、专业骨架、60–80 个种子案例、低保真原型和硬件快速基线；0B（约 2–4 周）完成量规收敛、资料授权盘点、指标口径、容量验证、可交互原型和对接方案。专家资源未到位时不得压缩 0B 后直接开工。
...
- 在合同已覆盖阶段 4 的前提下，冻结三条受控多智能体流程、图片/音频/短视频模态边界、微调双路径验收和 G4 最终交付物。
- 单独确认小程序和多人实时协同是否属于合同，不与已经确认的多智能体、多模态范围混淆。
...
- 对 OCR、ASR、VLM、Agent 调用放大和训练/批量评测资源做初始容量估算，不能把当前 27B 短问答能力视为阶段 4 容量证明。
- 确认备份、恢复、监控、日志和网络边界。

#### WP0.7 原型与平台对接

- 完成教师任务首页、成果工作区、引用卡、诊断面板和版本差异原型。
- 用真实教师完成可用性走查。
...
- 60–80 个种子案例完成审核，关键指标的口径和 G1 扩展计划明确。
- 首批资料授权和更新责任明确。
- 至少一个本地模型在样板任务上达到最低可用基线。
- GPU、存储、备份、网络及身份接口得到确认。
- 原型经代表性教师走查，核心旅程无阻断问题。
- 客户签字确认阶段 0–4 范围、微调双路径验收、候选指标、试点名单和投标演示故事线。
- `base-spark` 端口、网络、标准模型资产、vLLM arm64 镜像及 Tailscale HTTPS/ACL 责任明确；vLLM 有 Go/No-Go 结论，双环境 D1–D3 可执行。

若未通过 G0，只允许继续做资料整理、原型和技术验证，不进入大规模功能开发。

## 10. 阶段 1：可信底座与纵向样板

### 10.1 阶段目标

建设生成和诊断共用的领域、任务、知识、模型和版本底座，并先打通一个“课程依据映射—单元蓝图—课时设计—实施反馈”的完整纵向样板。建议围绕一个高中议题式主题完成上述四层样板，而不是只做一份课时教案；最终主题和学段由阶段 0 决定。
...
- 模型、提示词、规则、知识库、Schema 和导出模板版本记录。
...
- 配置 Tailscale Serve HTTPS，完成 `virtus` 浏览器登录、SSE、纵向样板、引用和导出验证，并限制集成环境只对研发/测试身份可见。
...
- 自动复用教师常用班情和模板，但每次允许确认和覆盖。
- 检测课时、课型、目标、资源和用途之间的冲突。
- 所有推测显示为“假设/待确认”。

#### WP2.2 结构化生成工作台

- 先生成课标对齐卡和设计蓝图，再分段生成正文。
- 编辑目标、任务、活动、证据和评价映射。
...
- 冻结至少三条受控多智能体业务流程、Agent 角色注册表、工具白名单、最大轮次、超时和调用预算。
- 建立授权多模态样本，冻结图片区域、页码、音频/视频时间点的证据标注规范。
- 完成微调候选数据质量报告和错误归因，证明剩余问题来自模型行为而非 RAG、规则、Schema 或交互。
...
根据网页端真实使用数据决定是否建设小程序。若建设，首期只承担：
...
阶段 4 是合同硬交付阶段，建议周期 14–18 周，以 G4 作为合同最终验收门。多智能体和多模态必须形成可运行、可评测、可降级的产品能力；微调必须完成数据、实验和 Go/No-Go 决策闭环，但不承诺不合格的微调模型进入生产。
...
- 每个 Agent 配置最小工具权限、最大轮次、超时、token、并发和调用预算。
- Agent 冲突必须回到证据、专家规则或人工确认，不能简单多数投票。
- 保存结构化输入、输出、证据、工具操作和版本，不展示或保存模型隐藏思维链。
- 公开发布、规则变更、高风险结论和外部写操作继续要求人工批准。
- 任一 Agent 失败、超时或预算耗尽时，可以退回阶段 3 单工作流，不得造成成果丢失或重复写入。
...
| 范围持续膨胀 | G0–G4 决策门、合同能力规格和明确非范围 | 保留受控多智能体、多模态和微调闭环，其他新增需求走变更流程 |
...
- Agent DAG、工具权限、轮次/预算、冲突、幂等、超时和单工作流回退测试。
...
本轮已同步根 README、架构说明、基础设施说明、两份 2026 扩展文档和客户最终方案的核心口径。各文档必须区分“当前问答 MVP 已实现能力”与“阶段 0–4 目标能力”，并以本计划作为范围、阶段和验收的主依据。对齐项如下：

1. 旧“2026 上半年/下半年”“一期/二期/三阶段”统一映射为阶段 0–4，G4 为合同最终验收。
2. 轻量诊断和修改对比前置到阶段 1–2；“诊断不等于评分”保持一致。
3. 多智能体、多模态和微调评估闭环统一为阶段 4 合同交付，并保留微调路径 A/B。
4. 正式和最终验收默认 vLLM；Ollama 仅作前期开发、过渡和可见降级，OpenClaw 不进入鲁韵链路。
5. 模型资产以标准权重/版本/哈希/Tokenizer/Template 为主，不以 Ollama tag 作为唯一来源。
6. `base-spark` 采用 `luyun-int`/`luyun-demo` 双环境，每个开发节点部署、`virtus` 验证后再晋级。
7. 对外材料明确“当前已完成 / 分阶段交付 / 条件式能力”，不得用目标架构描述当前实现。
8. 资料版权、有效期、更新责任、审核 SLA、容量、监控、备份、恢复和降级继续在阶段 0 锁定。

代码与数据层仍有一项进入开发前待处理：核对并修正 3 个种子案例的文件名与内容错位；本轮仅做文档对齐，不修改案例数据。

## 18. 进入开发前需要确认的决策

以下事项未确认前，本计划保持 `待确认` 状态：

合同覆盖阶段 0–4、正式/验收默认 vLLM、Ollama 过渡/备用，以及按节点双环境持续部署均已由用户确认，不再作为待决项。仍需确认：
...
18. Tailnet 管理员、Tailscale HTTPS/ACL 审批人、`virtus` 演示账号和允许访问的设备范围。
19. `base-spark` vLLM 的固定版本、标准模型资产、量化方式、最大上下文、显存上限，以及 D0 Go/No-Go 阈值；OpenClaw 不进入候选方案。
20. 演示账号、脱敏资料、黄金脚本、并发/性能目标，以及故障时是否允许明确披露的预置/缓存结果。
21. 小程序是否为合同硬范围；建议继续采用阶段 3 数据驱动决策，不与多智能体、多模态混为一项。
22. 是否需要单一诊断分数；建议全周期明确不提供。
23. 41–55 周周期、阶段 4 的 11–14 人高峰团队和客户侧配合资源。
24. `luyun-int` 与 `luyun-demo` 的最终 loopback 端口、Tailscale Serve 映射、研发访问名单和稳定演示晋级审批人。

## 19. 建议的批准方式

本计划建议按以下顺序确认：

1. 先确认第 0、2、3、4 节：产品目标、范围、专业模型和诊断边界。
2. 再确认第 5、6、7 节：模型能力补偿、评测和治理原则。
3. 单独确认第 7.5–7.9 节：base-spark、Tailscale、Virtus、模型隔离和投标演示验收。
4. 再确认第 8–13 节：41–55 周合同周期、阶段 0–4、增强试点、微调双路径和 G4 最终验收。
5. 最后确认第 18 节的开发前决策，并形成签字版 v1.0。

在 v1.0 获得确认前，不创建数据库迁移、不修改接口、不引入新依赖、不开始功能开发。
