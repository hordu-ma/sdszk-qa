...
> - 版本：v1.4（结构与文风修订：历次实施/部署记录移至《实施记录附录》，全文语言平实化；合同范围、阶段归属、G0–G4 门槛、全部阈值与禁止项与 v1.0 签字版一致）
> - 日期：2026-07-19（v1.0 签字版 2026-07-15；v1.1 平实化 2026-07-16；v1.2 自助开发模式 2026-07-17；v1.3 投标演示优先 2026-07-19）
> - 状态：签字版 + 自助开发/投标演示优先模式生效。客户与专家暂不参与期间，外部签字项按 §0.5 冻结为补签清单，工程按内部代理决策先行；G0/G1 外部签字仍是最终验收前提
> - 适用范围：鲁韵思政大模型的教学设计生产、诊断、产品 Skills、核心用户 Memory、受控多智能体、多模态、模型专项优化及投标演示部署。用户注册与身份认证的规范见 §2.6，实现落点不在本仓库
...
- 问答 MVP 与阶段 1A 首个可部署增量的代码、架构、测试和部署文档。
- `2026-product-extension-development-suggestions.md`（方向稿）与 `2026-product-extension-feature-spec.md`（能力图/招标素材），均非排期依据。
- `2026-luyun-external-solution-v1.0.md` 及其 Word 发布件（对外表达版）。
- 客户核心反馈：产品必须体现"教学与课程论"体系，重点是教学设计生产和诊断的专业可信度。
- 2026-07-15 工程评审与用户体系决策：纳入产品 Skills 与核心用户 Memory；确立纵向样板优先、阶段 4 能力门禁化、文档驱动开发；用户体系采用"注册/认证"两级并由思政课平台用户管理实现（§2.6）。
- 2026-07-15 签字前审计：小程序确认为合同硬交付（阶段 3 轻量端）；补入合规适用性、生成内容标识、评测四库隔离、独立红队、Data/System Card、持续评测与 RTO/RPO。

客户的核心顾虑成立：单纯部署开源大模型做不了专业教学设计与诊断。本项目不采用"开源模型 + 一段长提示词 + 直接输出完整教案"的建设方式。

可靠的教学成果不来自单个模型，而来自组件组合：权威课程知识、课程教学结构模型、专家规则、受控产品 Skills、核心用户受控 Memory、经专项评测的本地模型，以及教师与专家反馈闭环。
...
5. 合同范围覆盖阶段 0–4，以 G4 为最终验收门。阶段 4 的多智能体、多模态与微调评估均为能力门禁式交付（可运行样板 + 评测 + 降级/No-Go）；不合格能力不为形式交付强行上线。
6. 正式环境和最终验收默认 vLLM；Ollama 仅作前期开发、兼容性验证和明确标注的备用 Provider；业务代码不绑定任一推理引擎。
7. `base-spark` 采用集成环境与稳定演示环境双轨部署；每个可验收节点先完成部署、`virtus` 端到端验证和回滚检查，再晋级稳定演示版本。演示不抢跑未完成的教学能力，不用原型冒充已实现后端。
8. 产品 Skills：教师任务以版本化、有 Schema、有权限与审计的受控技能暴露；不做开放插件市场或自由 Agent 工具箱；阶段 4 Agent 复用同一批产品 Skills。
9. 核心用户 Memory：为教师与专家建设显式、可删、可审计的教学工作记忆；禁止思想侧写、能力评级与绩效画像；学生不做生成式 Memory。
10. 纵向样板优先：阶段 1–2 只打通一个学段/课型的完整闭环，不并行铺开十二大模块；工程顺序固定为 §5.4.1。
11. 文档驱动开发：实现、验收、代理交付均以本文阶段工作包与决策门为准；`.github/skills` 仅服务工程动作，不构成第二套产品需求。
12. 用户身份两级制（平台侧实现）：注册用户 / 认证用户，见 §2.6；本仓库不实现注册与 KYC，仅消费平台下发的身份声明与会话。
13. 小程序为合同硬交付，归属阶段 3、实施后置到全计划最后：先完成阶段 1–4 桌面 Web 核心闭环、增强能力和投标演示基线，再建设轻量端（统一登录、可信轻问答、收藏/历史、通知、任务进度、跳转网页续办）；完整备课工作台、Memory 全量管理和后台审核不进小程序。
...
16. 自助开发模式（2026-07-17 起）：执行规则见 §0.5。
17. 投标演示优先执行轨道（2026-07-19 起）：执行原则见 §0.6。
...
历次实施、部署与收口的详细证据在《实施记录附录》（`2026-implementation-log.md`）；本文只保留现状摘要。

### 0.3 假设/待确认

已确认：

- 合同建设范围覆盖阶段 0–4，以 G4 作为最终验收门。
- 投标演示环境部署在 `base-spark`，由同一 Tailnet 内的 `virtus` 浏览器访问；开发按节点持续部署，保留集成环境和稳定演示环境双轨，未经门禁不得用开发版本覆盖稳定演示版本。
- 正式环境和最终验收默认 vLLM；`base-spark` 的 Ollama 仅作过渡开发和备用 Provider，需在 D0 完成 GB10/arm64 上的 vLLM 专项验证。
- 产品 Skills 与核心用户 Memory 纳入阶段 0–4 开发计划（§2.5）；Memory 是工作记忆而非通用用户画像。
- 用户体系采用 §2.6 两级分级，开发落点为思政课平台用户管理；过渡期不以"自填身份证号且不核验"作为实名，默认不采集未核验身份证号。
- 小程序属于合同硬交付，归属阶段 3，边界为轻量访问和跨端续办；实际实现后置到全计划最后。
- 2026-07-17 起进入自助开发模式；客户、专家与资料授权相关决策暂缓，由内部代理决策先行并留痕待复核（§0.5）。

假设：

- 合同全周期主要用户是一线思政课教师、教研员和中心专家，不直接面向学生开放生成式对话。
- 最终生产环境坚持校内部署，本地模型不得静默切换到外部服务；`base-spark` 仅作投标演示和研发验证环境，不替代客户生产验收环境。

待确认：

- 山东师范大学信息中心可用 GPU、显存、CPU、内存、存储、备份和网络条件。
- 思政课平台用户管理的系统名、接口契约、token/claims 字段与对接方式（§2.6.5）；统一身份、组织、资源和审计接口与此合并推进。
- 教材、教参、中心资源及山东地方资源的数字化授权范围。
- 四学段专家的参与方式及审核时限。
- 周期和候选验收阈值需在阶段 0 基线完成后共同锁定。
- 投标演示日期、时长、必须现场展示的能力和评委观看方式。
- 阶段 1 首批产品 Skills 的最终命名与客户话术（建议清单见 2.5.1）。
...
背景：客户与四学段专家暂不参与项目，而阶段 1 剩余工作全部卡在外部输入（真实资料、专家金标、正式选型、外部签字）上。为避免停滞，自 2026-07-17 起进入自助开发模式。本节只调整执行方式；范围、阶段划分、G 门标准和全部禁止项不变。

执行规则：

1. **内部代理决策。** 原需客户/专家确认的输入由项目负责人内部决定并立即生效，以"内部代理决策"字样记入实施记录。外部参与恢复后逐项提交复核，允许被推翻并按台账替换。
2. **外部补签清单。** G0/G1 中需要客户或专家签字的条目整体冻结为补签清单（正文见《阶段 1 工程冻结基线》§8 与《阶段 1 模拟信息替换台账》§2），不再阻塞工程开发；G0、G1、阶段 1A/1B 整体状态保持"未完成"，直至补签闭合。
3. **内部金标 v0。** 工程团队自研 120–160 个案例建成专用数据集（key：`stage1-internal-gold`），走完整批量导入、双评、第三人仲裁治理流程，标定内部工程阈值。该集只用于工程回归、流程演练和阈值标定；禁止改名、改来源或改审核状态冒充 `customer_provided`/`expert_authored`。
4. **内部里程碑 M0-int / M1-int。** 与 G0/G1 条目一一对应的内部验收门：不涉及外部签字的条目按原标准执行，涉及外部签字的条目替换为对应内部代理产物。通过 M-int 是继续下一阶段工程开发的依据。
5. **反虚报约束（全文通用，各处不再重复）。** `CONTENT_MODE` 保持 `synthetic`；演示、文档和界面不得把内部代理决策、内部金标或内部阈值表述为客户确认、专家签字或专业验收；工程命中率不得解释为教学质量指标；任何 M-int 通过、内部收口或部署验收都不改变对应阶段和 G 门"未完成"的状态。
6. **退出条件。** 客户或专家恢复参与后，按《阶段 1 模拟信息替换台账》§3 顺序替换模拟项，内部代理决策逐项复核；M-int 结果作为外部验收的输入而非结论。

自助开发优先级（阶段 1 内，按序推进，遵守"每周一个纵向增量"）：

1. 检索质量调优：分析模拟回归失败根因，设定并达成内部命中率目标。
2. 内部金标 v0：自研案例扩充至 120–160 例，完成双评/仲裁全流程，产出《内部阈值标定报告 v0》。
3. 引用精细化：页码/段落级定位、资料有效期与资料不足策略。
4. 诊断规则字典 v2（内部版）：三个工程诊断维度扩展为可配置规则字典，保持非评分约束和防护测试。
5. 工作台体验：专业编辑、可用性走查与无障碍适配的内部版本。

状态（2026-07-19）：优先级 1–5 已全部完成，M1-int 内部门通过（见 §10.4 与《M1-int 收口记录》）。

### 0.6 投标演示优先执行轨道（2026-07-19 策略调整）

本轨道解决客户与专家暂不参与造成的推进阻塞，服务于投标环节的可重复演示。它是阶段 0–4 的执行视图，不新增合同范围，也不降低 G0–G4 的外部验收门槛。

执行原则：

1. **内部代理替代外部输入。** 尚未获得的用户资料、专家规则、客户 Word 模板、平台接口和试点反馈，使用明确标记的 `synthetic` / `internal_authored` 数据、内部标准模板和代理决策继续开发；所有替代项进入补签/替换台账。
2. **先做可演示工程闭环。** 优先完成桌面 Web 的真实 API、版本、审计、降级、导出、演示脚本和双环境部署；演示只调用已实现的 Skills，不用静态原型冒充后端能力。
3. **用户参与改为后置验证。** 真实教师试用、专家双评、客户模板确认、平台统一身份和试点反馈不再作为开发前置条件；恢复参与后按替换台账逐项复核。
4. **按演示风险而非用户等待排序。** 先处理会导致演示中断、结果不可追溯、无法回滚或无法解释的工程风险，再扩展能力面；每个纵向增量仍必须经过 `luyun-int`、运行态验证、回滚检查和文档对齐。
5. **小程序最后实现。** 阶段 3 期间只保留小程序的接口、身份、通知和跨端续办约束；待桌面 Web 核心闭环、阶段 4 增强能力和投标演示基线稳定后，作为全计划最后一个实施包完成并验收。

当前执行顺序：

`WP2.3 证据化诊断 → WP2.4 内部评测/仲裁工程化 → WP2.5 安全、性能、恢复 → WP2.6 内部投标演示脚本与稳定基线 → 阶段 3/4 Web 能力和增强门禁 → 小程序后置收口 → 外部补签与 G0–G4 决策门`。

"内部投标演示完成"只表示工程链路可运行、可解释、可回滚，不等于真实教师可用性、专家专业确认、客户验收或 G 门通过。

## 1. 当前基线与建设差距

### 1.1 当前已实现能力

截至 2026-07-20，项目已从问答 MVP 前进到"问答兼容链路 + 阶段 1 工程样板 + 阶段 2 WP2.1–2.4 内部收口"。当前实现的完整清单以 `README.md`"当前状态/已实现能力"为准，要点：

- 问答链路：本地登录、JWT、主题、会话历史、经最小 ModelClient 的 SSE 流式问答（`luyun-int` 默认 vLLM 主链，Ollama 明示备用）、限流、Trace ID、结构化日志和操作审计。
- 领域与任务底座：Teaching Project/Version、Knowledge Document/Chunk、TaskRun、SkillRun、ModelCallAudit 及迁移；问答编排服务层；应用内异步任务（排队、取消、重试、重启恢复）。
- RAG：资料上传、后台解析、审核门禁、`pg_trgm + pgvector + Reranker` 语义混合检索、段落/页码定位引用、资料有效期、资料不足两级判定、Provider 故障显式降级；项目级知识索引版本、配置哈希与原子激活。
- 模型与评测：固定 vLLM 运行时镜像摘要与三类模型 revision 登记（工程候选）；版本化工程评测（冻结哈希、发布清单绑定）；来源门禁（`synthetic`/`internal_authored`/`customer_provided`/`expert_authored`）；批量导入、双评、第三人仲裁与占位案例防冻结；内部金标 v0（140 例）与《内部阈值标定报告 v0》。
- Skills 与 Memory：统一 Skills 运行时（权限、Schema、审计、停用与降级契约），八个已注册 Skill；显式 Memory（偏好、班情、钉选、注入确认、导出、一键清除、注入审计）。
- 纵向样板与阶段 2 增量：高中议题式"对齐卡 → 蓝图 → 课时分块 → 非评分诊断 → 结构化 Word 导出"闭环；结构化专业编辑与不可变版本；WP2.1 专业输入与冲突规则；WP2.2 章节锁定、局部重生成、多成果派生与字段级差异；WP2.3 结构校正、证据化诊断、逐条决定与仅采纳项修订。
- 部署：`base-spark` 的 `luyun-int`/`luyun-demo` 双环境独立 Compose、Tailscale Serve HTTPS、同镜像晋级、备份与回滚基线。

上述状态是实施快照，不改变本计划的范围、工作包和 G 门。当前结论：**阶段 1 模拟工程闭环与双环境晋级可运行，M1-int 与 WP2.1–2.4 内部收口完成；阶段 1A、阶段 1B、阶段 2、G0、G1 整体均未完成。**

### 1.2 当前尚未具备的关键能力

- 统一身份、组织同步和完整角色权限（目标由思政课平台用户管理提供，本仓对接消费；§2.6）。
- 完整可信 RAG 的外部输入：正式模型选型、资料授权和专业阈值尚未冻结（语义工程链已实现）。
- 经专家确认的课程层、单元层、课时层完整结构化教学成果（当前只有高中议题式技术样板）。
- 自由编辑与可视化富文本差异（当前为结构化编辑、结构化差异和 Word 导出）。
- 经专家签字的完整诊断量规与规则字典（当前为内部版规则字典）。
- 专家签字的完整阶段 1 Skills 目录、正式配额和质量策略。
- Memory 保留/删除 SLA 与授权共享模板。
- 独立 Worker/队列、共享缓存、通用幂等和高峰降级（当前仅应用内后台任务基线）。
- 模型、提示词、规则、知识库、Skill 和成果版本的完整统一追溯（当前已有部分对象与调用审计）。
- 真实专家评测集、专业发布阈值和教师采用反馈（治理工具已上线，尚无 120–160 个真实专家金标）。
- 真实教师可用性研究与富文本编辑（内部版工作台已支持结构化编辑与基础无障碍）。
- 管理端、审核流、资源库、区域运营看板和阶段 3 合同归属的小程序轻量端（实现后置）。
- 受控多智能体、多模态证据、微调流水线（阶段 4 门禁交付）。
- 正式专业演示版本、完整演示降级包与 Virtus 人工验收（模拟工程双环境与回滚基线已完成）。
- 经专业评测冻结的标准模型资产清单与 Provider 一致性回归（工程登记与运行基线已实现）。
...
- "已实现、一期承诺、后续愿景"边界不清。
- 生成和诊断以功能描述呈现，缺少统一的课程教学领域模型。
- 验收偏重"能否生成"，缺少专业正确性、证据充分性、教师采用和时间节省指标。
- 诊断放在第二阶段，一期只有"生成"没有"修改闭环"。
- 多智能体、小程序、研修、资源管理和看板同时开工，会稀释核心体验。
- 没有显式 Skills 与 Memory 设计时，系统会退回"长会话 + 隐式上下文"，无法审计、无法复用、难以验收。

因此，本计划把轻量诊断、局部修改、版本对比、标准导出、首批 Skills 与 Memory 最小集前置到阶段 1–2，再在阶段 3–4 扩展四学段、一体化和增强能力，最后实现小程序轻量端。
...
4. 先服务层后功能：阶段 1 启动时先把 LLM 调用与编排移出路由。
5. 演示不抢跑：D0–D3 只保证可访问与真实已实现能力；未完成能力必须标记为原型/预置。
6. 桌面优先工作台：备课/诊断/版本/导出以桌面网页为主；小程序按轻量硬交付边界建设，不与阶段 1–2 核心闭环捆绑。
...
- 多人实时协同编辑。
- 完整小程序工作台（小程序只交付轻量端；复杂编辑、完整 Memory 管理、审核和管理看板由网页端承担）。
- 自动将教师成果发布到公共资源库。
- 生产环境外部模型兜底。
- 大规模通用知识图谱建设。
...
阶段 0–3 只预留 Agent、多模态资产和训练数据血缘所需的结构与接口；受控多智能体、多模态教学证据服务和微调决策包在阶段 4 按门禁交付。
...
对外与合同表述：步骤 1–4 称**注册 / 身份信息登记**，不称"实名认证"；步骤 5 称**认证用户**或**身份核验通过**，具体子类型在 claims 中区分。
...
2019 年学校思想政治理论课教师座谈会提出的"八个相统一"是专家规则的重要政策来源。产品需要由分学段专家把它转化为可观察的设计证据；不能由模型机械打标签、自动评分，也不能把八项都设成每节课的同质化硬指标。规则重点检查政治性与学理性、价值性与知识性、理论性与实践性、教师主导与学生主体等关系是否在具体教学设计中得到恰当体现。
...
技术选型在阶段 0 形成 ADR。本计划不提前锁定具体生成模型、Embedding、Reranker 或任务队列框架。
...
- 三个案例曾出现文件名与内容错位（劳动教育/法治教育/国家安全互相错位）。

阶段 0 必须修正命名和标签，再扩展为专家评测案例。金标案例应逐步脱离 `reference_answer.primary` 单句方向描述，改为：输入条件、必引用、禁止项、量规条目、可接受差异与专家解释。

状态（2026-07-16）：错位文件已重命名并接入 `make validate-cases` 一致性校验；`reference_answer.primary` 的量规化改造仍在阶段 0 完成。详见《实施记录附录》§1。
...
以下为 2026-07-16 时点核验结果；后续实施前必须重新检查，最新部署状态以《实施记录附录》和 `src/infra/README.md` 为准：

| 项目 | 当时状态 | 对计划的影响 |
| --- | --- | --- |
| `base-spark` | Tailscale 在线，MagicDNS 为 `base-spark.tail84088a.ts.net` | 可作为投标演示主机 |
| `virtus` | 与 `base-spark` 同一 Tailnet；已完成真实浏览器登录和 SSE 问答验证 | 可作为研发验收终端；新增功能仍需在该机浏览器做人工界面复核 |
| Tailscale Serve | `https://base-spark.tail84088a.ts.net/` → `http://127.0.0.1:8088`，HTTPS `/healthz` 通过 | Serve 链路已完成；持续核验 ACL/Grant 只覆盖指定 Tailnet 身份，保持 Funnel 关闭 |
| 计算资源 | `aarch64`、NVIDIA GB10、约 121 GiB 内存、约 769 GiB 可用磁盘 | 已承载三类固定版 vLLM 工程候选；长上下文/并发、多 Agent、多模态和训练容量仍须专项验证 |
| vLLM | 固定摘要的 vLLM `0.18.0`，生成/Embedding/Reranker 固定模型 commit；健康、Chat、结构化 JSON、SSE、512 维向量、重排和服务重启已验证 | 形成 D0 工程兼容性基线；正式选型、长上下文/并发/容量对比和 Go/No-Go 尚未完成 |
| Ollama 本地模型 | 已有 `qwen3.5:27b` 和 `gemma4` | 只作前期开发、过渡 Provider 和明确标注的备用；现有 tag 不能代替正式模型资产登记 |
| OpenClaw | 日常控制面独立运行并使用本地模型 | 不进入鲁韵调用链路，不复用 Operator Token、会话、渠道、Workspace 或工具权限 |
| 当前短请求样本 | 经现有 Ollama/OpenClaw 链路的单次冷路径约 28.49 秒、预热后约 2.46 秒；两路短请求约 2.67/4.43 秒 | 仅证明短问答初步可行，不是 vLLM、长教案、多 Agent 或视频任务的容量证据 |
| 宿主端口与网络 | `9000/9001` 已被其他容器占用，`172.29.0.0/24` 属于其他项目网络 | 必须采用专用 Compose 项目、网络、卷和端口预检 |
| 双环境基础 | `luyun-int`/`luyun-demo` 已同镜像部署，两套 PostgreSQL、MinIO、vLLM 端口、卷和 Secret 隔离且健康 | 已形成模拟工程晋级和金标治理工具基线；真实资料、专家回归、ACL/Grant 与 Virtus 人工验收前不得标记为正式专业演示版本 |

当前 API 已通过最小 ModelClient 使用逻辑模型名、Provider 标识和 Provider 模型 ID。Provider 能力登记、认证隔离、任务路由和一致性回归仍未完成；"能返回文本"不等于模型服务已经统一，该差距须通过完整 ModelGateway 和 Provider Adapter 解决。
...
- D0 在 `base-spark` 验证 vLLM 后，将 vLLM 作为稳定演示环境默认 Provider；验证未通过前允许 Ollama 承接过渡演示，但界面、验收记录和运行手册必须明确能力状态。
- Ollama 复用同一模型家族或经登记的标准模型资产，不直接把现有 `ollama pull` tag 当成 vLLM 模型目录；两种 Provider 的模型哈希、量化、Tokenizer、Template 和评测结果分别登记。
...
首个可从 `virtus` 访问的技术基线建议 2–3 周；达到可用于正式投标彩排、包含故障降级和恢复演练的完整 D0–D3 基线，建议 3–5 周。该轨道可与阶段 0/1 并行；它只解决部署、接入和演示保障，不提前尚未开发的教学能力。
...
状态（2026-07-19）：D0/D1 工程基线已建立，D2 技术链路已验证；正式模型选型 Go/No-Go、新增功能 Virtus 人工验收、正式监控与两次完整彩排未完成。详见《实施记录附录》§2。

D0–D3 的工期是各工作包的投入窗口，不是可无条件压缩的串行总和。D0 的盘点、故事线和 Tailnet 审批可并行，D2 的权限准备可与 D1 后半段重叠；但 D3 必须在可部署版本和跨设备链路稳定后执行。若投标日期早于 3 周，必须缩减现场实时能力并明确采用 D-L2/D-L3，不以取消恢复和彩排测试换取工期。

首个演示基线只能展示当时真实完成的功能；尚未完成的页面只能作为明确标注的交互原型，不得伪装为已实现后端能力。
...
- 建议合同完整建设周期：46–63 周，覆盖阶段 0–4；专家、资料、平台接口和硬件均按期到位时争取 46–55 周，长期运营不计入。
- 阶段 0–3 假设 8–10 人全职核心团队（产品、课程教学、AI、后端、前端、QA），另有共享 DevOps、1–2 名内容运营和四学段兼职专家组。
- 阶段 4 高峰建议扩充到 11–14 人，增加 Agent/LLM 平台、多模态、训练/MLOps、数据质检和隐私安全能力；若不扩充，阶段 4 按 20–26 周、合同总周期按 52–69 周估算。
...
- 46–63 周按阶段 0–2 的 22–29 周、阶段 3 的 8–12 周和阶段 4 的 16–22 周估算；投标演示先行轨道与阶段 0/1 并行，原则上不额外增加总周期；小程序后置通过调整阶段 3/4 内部顺序消化。
- 阶段 3 的 8–12 周先完成四学段、一体化、审核、组织和运营 Web 能力，只冻结小程序接口与跨端协议；真实试点与平台接口按补签清单后置。
- 阶段 4 的 16–22 周先完成多智能体、多模态、微调评估和投标演示 v4；小程序轻量端在核心能力稳定后作为全计划最后实施包，条件不满足时不得用压缩验证替代延期。
...
| 阶段 3：一体化特色与生产运营 | 8–12 周 | 扩展四学段、一体化、审核、组织和运营 Web 能力；冻结小程序接口与跨端协议 | 生产候选、中心特色能力、运营制度、小程序后置实施包规格 | G3：是否具备受控开放条件 |
| 阶段 4：智能协作与多模态增强 | 16–22 周 | 交付门禁化多智能体（复用 Skills）、多模态证据、微调评估和投标演示 v4；核心稳定后最后实现小程序 | 合同最终候选 v4.0、增强能力或正式 No-Go/缩范围包、小程序轻量端和最终验收包 | G4：是否完成合同最终验收 |
...
阶段 0 分为两个连续子阶段：0A（约 3 周）完成范围、专业骨架、60–80 个种子案例、低保真原型和硬件快速基线；0B（约 2–4 周）完成量规收敛、资料授权盘点、指标口径、容量验证、可交互原型和对接方案。专家资源未到位时不得压缩 0B 后直接开工。
...
- 冻结三条受控多智能体流程、图片/音频/短视频模态边界、微调双路径验收和 G4 最终交付物；多智能体/多模态与微调同为门禁式交付。
- 冻结阶段 1 产品 Skills 目录（2.5.1）与 Memory 边界（2.5.2）。
- 冻结小程序页面清单、平台登录、消息通知、跨端续办协议和发布主体；多人实时协同仍需单独确认，不与已确认范围混淆。
...
- 完成教师任务首页（按 Skill 入口）、成果工作区、引用卡、诊断面板、版本差异与 Memory 管理原型。
- 原型交互按桌面优先工作台设计；可附移动浏览说明，但不把完整备课交互绑在小程序上。
- 用真实教师完成可用性走查。
- 明确 SSO、组织、角色、资源和审计接口。
- 与思政课平台用户管理对齐 §2.6：注册/认证状态机、claims 字段、权限差；实现任务单开在平台用户管理项目，不进本仓 backlog。
- 形成字段映射、接口责任方和联调环境计划。

#### WP0.8 投标演示与阶段 4 前置设计

- 完成 D0 环境盘点、演示故事线、能力状态标签、端口/网段登记、Tailnet HTTPS 与 ACL 责任确认。
- 冻结 `base-spark` 双环境拓扑、专用 Compose 边界、vLLM 默认与 Ollama 备用 Adapter 方案、版本晋级门禁和 `virtus` 跨设备验收脚本。
...
- 60–80 个种子案例完成审核，关键指标口径和 G1 扩展计划明确；已知错位种子案例已修正。
...
自助开发模式（§0.5）期间：本门涉及外部签字的条目冻结为补签清单，工程按 M0-int 内部产物推进。

## 10. 阶段 1：可信底座与纵向样板

### 10.1 阶段目标

建设生成和诊断共用的领域、任务、知识、模型、产品 Skills、Memory 和版本底座，并先打通一个"课程依据映射—单元蓝图—课时设计—实施反馈"的完整纵向样板。建议围绕一个高中议题式主题完成四层样板，而不是只做一份课时教案；最终主题和学段由阶段 0 决定。工程顺序强制遵循 §5.4.1。
...
状态（2026-07-19）：阶段 1 工程样板与自助开发优先级 1–5 已完成，M1-int 内部门通过；真实资料替换、真实专家金标、专业模型冻结、Virtus 客户人工验收及 G0/G1 外部签字仍未完成，阶段 1A/1B 整体保持"未完成"。九个增量的实施与部署证据见《实施记录附录》§3–§14，工程边界见《阶段 1 工程冻结基线》。
...
- 预留 `AgentRun`、工具授权、Agent 中间结构化产物、多模态资产、页码/坐标/时间戳、识别纠错和数据血缘对象，但阶段 1 不宣称这些增强能力可用。
...
- 建立 Provider Adapter 契约，支持默认 vLLM 与过渡/备用 Ollama Provider 切换；教学业务代码只使用逻辑模型名；OpenClaw 不进入调用链路。
...
自助开发模式（§0.5）期间：本门涉及真实专家金标、专业阈值和外部签字的条目冻结为补签清单，工程按 M1-int 内部产物推进。

状态（2026-07-19）：M1-int 内部门结论为**通过**，允许进入阶段 2 内部工程；`stage1-workbench-ux-20260719-r1` 已同镜像晋级 `luyun-demo` 并完成回滚演练。证据见《阶段 1 M1-int 内部门收口记录》与《实施记录附录》§14。
...
状态（2026-07-19）：内部工程已收口。`skill.confirm_professional_input` v1.1.0 + 规则集 `stage2-input-conflict-v2`：分步专业输入、显式保存班情档案/常用模板、有序可注册冲突规则字典、已确认输入作为 `alignment_card` 服务端单一事实源。真实教师可用性确认与客户/专家补签未完成。详见《WP2.1 收口记录》与《实施记录附录》§15。
...
状态（2026-07-19）：内部工程已收口。`skill.generate_section` v1.1.0 + 规则集 `stage2-structured-gen-v1`：服务端章节/成果锁定、字段级局部重生成、来源版本冲突检查、字段级差异、历史恢复不删旧版本、五类配套成果派生、`word-standard-v2` 汇总导出。客户专属 Word 模板实物与真实教师长文档验证未完成。详见《WP2.2 收口记录》与《实施记录附录》§16。
...
状态（2026-07-19）：内部工程已收口。结构识别预览与教师校正确认；`skill.diagnose_artifact` v1.1.0 + 规则集 `stage2-evidence-diagnosis-v1`（每条诊断项含原文位置、规则依据、证据、影响、建议、示例改写）；逐条决定记录为 `signal_level=L4`、`authorized_for_training=false`；`skill.apply_revision` v1.0.0 只应用采纳项并尊重字段锁定。分学段量规仍待专家确认。详见《WP2.3 收口记录》与《实施记录附录》§17。
...
状态（2026-07-20）：内部工程已收口。第一增量交付发布清单变更检测、内部阈值阻断与运行间回退对比；第二增量交付 SkillRun/诊断抽检队列（双评—分歧—第三人仲裁，复用金标状态机，含量规修订反馈）与教师 L4 决定按规则维度汇总（`spot_check_items`/`spot_check_reviews`，迁移 `n3b4c5d6e789`），当前部署 `stage2-wp24-spotcheck-20260720-r1`。复核人为内部代理，全部信号 `authorized_for_training=false`；真实专家抽检与分学段量规确认仍属补签清单。详见《WP2.4 收口记录》与《实施记录附录》§18–§19。
...
状态（2026-07-22）：增量 1（安全测试与输入防护，`stage2-wp25-security-20260720-r1`）与增量 2（试点组织白名单与最小 RBAC，`stage2-wp25-orgrbac-20260722-r1`）已完成并部署 `luyun-int`（均含回滚演练）。增量 1：上传真实内容校验（可执行魔数/PDF 头/DOCX 结构与宏载荷/UTF-8 文本，415）、提示注入数据定界与历史角色白名单、跨用户越权与角色边界安全回归套件。增量 2：新增 `organizations` 与 `users.organization_id`（迁移 `o4c5d6e7f890`，nullable+回填），试点白名单门禁作用于整个工作台，最小 RBAC 收敛到 `services/rbac.py`，reviewer 跨组织隔离覆盖全部特权跨用户路径（评测/抽检/L4/资料审核），组织管理 `/api/organizations` 限平台 admin。平台 admin 组织无关且可跨组织为有意缺口。模型/RAG/队列分级降级、备份/任务/版本回滚演练、试点并发指标为 WP2.5 剩余增量。安全防护为工程缓解措施，不代表安全审计或渗透测试结论。详见《WP2.5 收口记录》与《实施记录附录》§20–§21。
...
#### WP3.8 小程序轻量端规格冻结（实现后置，合同硬交付）

小程序仍归属阶段 3、仍为合同硬交付，但实现顺序后置到阶段 4 核心能力和投标演示基线完成之后。阶段 3 本工作包只冻结页面清单、身份/通知/跨端续办协议、审计和发布责任，不与阶段 1–2 的 Skills/Memory 核心闭环抢资源。最终实现作为阶段 4 最后实施包执行。功能边界固定为：
...
在客户与专家未恢复参与期间，本工作包先交付内部代理版：使用内部样板、合成/内部自研评测、预置演示数据和可重复脚本完成工程、降级、回滚和演示材料；真实 16 名教师、8 名专家和连续 4 周试点改为外部补签后的验证包，不阻塞内部投标演示开发。

### 13.7a WP4.7 小程序轻量端最终实施包（全计划最后）

本工作包仍履行阶段 3 的合同归属，但实施顺序固定为全计划最后，必须在 WP4.6 的桌面 Web 核心、增强能力门禁和投标演示 v4 稳定后才开工：

- 接入平台统一登录及注册/认证 claims；本仓只消费平台声明，不实现注册、短信或 KYC。
- 实现可信轻问答、收藏/历史、通知、异步任务进度和跳转网页续办。
- 复用网页端 Skills、权限、审计、内容标识和降级契约，不复制完整备课工作台。
- 不在小程序承载大文档编辑、完整 Memory 管理、资源审核、管理看板或开放 Agent 工具。
- 完成小程序独立构建、设备兼容、权限/通知、跨端续办、数据删除、回滚和演示脚本验证；通过后才进入最终 G4 合同验收包。
...
阶段 4 高峰需要补充或明确兼任：Agent/LLM 平台工程 1 人、多模态工程 1–2 人、训练/MLOps 1 人、数据工程与标注质检 1–2 人、隐私安全 0.5–1 人。投标演示 D0–D3 期间 DevOps/安全必须有明确主责人，不能仅作临时兼职响应。
...
- `2026-product-extension-feature-spec.md` / `2026-product-extension-development-suggestions.md`：能力图与方向稿，页眉须声明非排期。
...
- 实施与部署证据：写入《实施记录附录》（`2026-implementation-log.md`）与对应收口记录，主计划只保留现状摘要。

### 17.2 版本变更记录

- v0.4：纳入 Skills、Memory、纵向样板优先、阶段 4 门禁化、文档权威与 §0.4 允许探针；旧"一期/二期"表述统一映射为阶段 0–4；对齐 README/ARCHITECTURE/扩展文档等。
- v0.5：新增 §2.6 用户注册与认证两级制；实现落点为思政课平台用户管理，本仓不开发注册与 KYC。
- v1.0（签字版）：签字前全量审计；小程序确认为阶段 3 合同硬交付（轻量端边界）；补入合规适用性、生成内容显隐式标识、投诉纠错、评测四库、独立红队、Data/System Card、供应链、持续评测和 RTO/RPO；阶段 1 拆为 1A/1B；合同周期调整为 46–63 周，不以压缩验证换工期；清理旧评分/医疗说明。
- v1.1：对签字版做语言平实化改写；未改动范围、阶段、验收门槛、阈值、表格数据、代码块和签署节，改写内容与 v1.0 逐节比对确认语义等价。
- v1.2：新增 §0.5 自助开发模式；外部签字项冻结为补签清单，工程按内部代理决策与 M0-int/M1-int 推进。
- v1.3：新增 §0.6 投标演示优先执行轨道，调整阶段 3/4 执行顺序与周期假设，小程序保留阶段 3 合同归属但实施移至全计划最后；不改变合同范围、阶段归属和 G0–G4 外部验收门槛。
- v1.4：结构与文风修订。历次实施/部署/收口记录原文移至《实施记录附录》，主计划改留现状摘要；合并散布的反虚报免责表述为 §0.5 第 5 条全局规则；§17–§19 压缩为版本记录与待决清单；全文语言平实化。范围、阶段、G0–G4 门槛、全部阈值、表格数据、代码块、禁止项和签署节与 v1.0 签字版一致。

## 18. 进入开发前需要确认的决策

以下事项未确认前，本计划总体保持 `待确认`；第 0.4 节允许项可执行。合同覆盖阶段 0–4、默认 vLLM、双环境持续部署、Skills 与 Memory 纳入计划、两级用户体系及平台侧实现、阶段 3 小程序轻量端硬交付均已确认方向，不再作为"做不做"的待决项。仍需确认：
...
20. 正式演示账号、脱敏资料、黄金脚本、并发/性能目标，以及故障时是否允许明确披露的预置/缓存结果；当前 `demo_teacher` / `demo_admin` 仅为内部测试账号。
21. 小程序页面清单、平台登录方式、跨端续办协议、消息通知能力和发布主体（合同硬范围已确认）。
...
v1.0 已按以下顺序确认并签字：先确认第 0、2–4 节（产品目标、范围、Skills/Memory、用户分级、专业与诊断边界），再确认第 5–7 节（架构、工程顺序、评测、治理与投标演示验收），最后确认第 8–13 节（周期、阶段 0–4、增强门禁、微调双路径、G4）与第 18 节优先 6 项。v1.0 确认前的开发限制见 §0.4。
