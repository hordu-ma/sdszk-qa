...
2026-07-19 当前集成候选：`luyun-int` 应用镜像 `stage2-wp24-gate-20260719-r1`（WP2.4 第一增量：评测回归门禁），API/Web 镜像 ID 为 `8a26431c4877...` / `af4bc5e303db...`，迁移保持 `m2a3b4c5d678 (head)`，发布前备份位于 `/home/pgx/backups/luyun-sizheng/20260719-stage2-wp24-gate-r1-predeploy/`。本轮无 Schema 变化，只重建 `luyun-int` API/Web，标准 Dockerfile 在线构建成功。真实链路验证：门禁接口对历史运行判定 `stale` 并列出 5 项清单变更；重跑 140 例内部金标与 64 例冒烟集后均 `promotable`（匹配率/Top-1 均 1.0、零回退）；vLLM 未降级、真实 SSE `[DONE]`、Tailnet HTTPS 通过。应用已实际回滚至上一收口镜像 `stage2-wp23-closure-20260719-r1` 后健康并恢复当前镜像，无需数据库降级。详细记录见《实施记录附录》§18；上一收口范围见《[WP2.3 收口记录](../docs/2026-stage2-wp23-closure.md)》。
...
早前增量已修复 MinIO healthcheck：旧配置 `mc ready local` 固定探测 `localhost:9000`，与 Base-Spark 的 `29000/30000` 实际监听不一致；当前检查通过 `MINIO_PORT` 生成目标地址，并在容器内展开已有凭据。回滚当前 WP2.4 门禁应用只需将 `luyun-int` 仓库外 env 的 `RELEASE_TAG` 改回 `stage2-wp23-closure-20260719-r1` 并重新创建 API/Web；本轮无数据库降级。数据损坏时从上述发布前备份恢复 PostgreSQL；MinIO 本轮未修改对象数据。`luyun-demo` 可用 `tailscale serve --https=8443 off` 撤销入口，并停止其 Compose project；普通停用不得加 `-v`。
