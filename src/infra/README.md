...
2026-07-20 当前集成候选：`luyun-int` 应用镜像 `stage2-wp25-security-20260720-r1`（WP2.5 第一增量：安全测试与输入防护），API/Web 镜像 ID 为 `95b5bc8a8151...` / `1bd060766861...`，迁移保持 `n3b4c5d6e789 (head)`，无 Schema 变化、无前端改动，只重建 API/Web。标准 Dockerfile 在线构建成功。真实链路验证：五类恶意文件上传 415 且正常 Markdown 202、跨用户资源 403/404、教师角色复核入口 403、匿名 401/403、注入话术真实 SSE 稳定并 `[DONE]`、`/auth/me` 不泄露口令散列、vLLM 未降级、本机与 Tailnet 两个 HTTPS `/healthz` 通过。应用已实际回滚至上一镜像 `stage2-wp24-spotcheck-20260720-r1` 后健康（无数据库变更）并恢复当前镜像。发布前备份位于 `/home/pgx/backups/luyun-sizheng/20260720-stage2-wp25-security-r1-predeploy/`。详细记录见《实施记录附录》§20；收口范围见《[WP2.5 收口记录](../docs/2026-stage2-wp25-closure.md)》。上一收口（WP2.4 抽检队列 + L4 信号汇总，迁移 `n3b4c5d6e789`）见《[WP2.4 收口记录](../docs/2026-stage2-wp24-closure.md)》与《实施记录附录》§18–§19。
...
早前增量已修复 MinIO healthcheck：旧配置 `mc ready local` 固定探测 `localhost:9000`，与 Base-Spark 的 `29000/30000` 实际监听不一致；当前检查通过 `MINIO_PORT` 生成目标地址，并在容器内展开已有凭据。回滚当前 WP2.5 安全应用只需将 `luyun-int` 仓库外 env 的 `RELEASE_TAG` 改回 `stage2-wp24-spotcheck-20260720-r1` 并重新创建 API/Web；本轮无 Schema 变化，旧镜像可直接运行，无需数据库降级。数据损坏时从上述发布前备份恢复 PostgreSQL；MinIO 本轮未修改对象数据。`luyun-demo` 可用 `tailscale serve --https=8443 off` 撤销入口，并停止其 Compose project；普通停用不得加 `-v`。
