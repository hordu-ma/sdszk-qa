...
2026-07-20 当前集成候选：`luyun-int` 应用镜像 `stage2-wp24-spotcheck-20260720-r1`（WP2.4 第二增量：抽检队列 + L4 信号汇总，WP2.4 内部工程收口），API/Web 镜像 ID 为 `3fe20bf742c9...` / `f94ec1381933...`，迁移升级至 `n3b4c5d6e789 (head)`（新增 `spot_check_items`/`spot_check_reviews`，仅新增表），发布前备份位于 `/home/pgx/backups/luyun-sizheng/20260720-stage2-wp24-spotcheck-r1-predeploy/`。标准 Dockerfile 在线构建成功。真实链路验证：诊断运行抽样入队、双评分歧、第三人仲裁与 resolved 结论、教师 403、项目级/全局 L4 维度汇总；vLLM 未降级、真实 SSE `[DONE]`、本机与 Tailnet 两个 HTTPS `/healthz` 通过。应用已实际回滚至上一镜像 `stage2-wp24-gate-20260719-r1` 后健康（新表向后兼容，无数据库降级）并恢复当前镜像。详细记录见《实施记录附录》§18–§19；收口范围见《[WP2.4 收口记录](../docs/2026-stage2-wp24-closure.md)》。
...
早前增量已修复 MinIO healthcheck：旧配置 `mc ready local` 固定探测 `localhost:9000`，与 Base-Spark 的 `29000/30000` 实际监听不一致；当前检查通过 `MINIO_PORT` 生成目标地址，并在容器内展开已有凭据。回滚当前 WP2.4 抽检应用只需将 `luyun-int` 仓库外 env 的 `RELEASE_TAG` 改回 `stage2-wp24-gate-20260719-r1` 并重新创建 API/Web；`n3b4c5d6e789` 仅新增表，旧镜像可直接运行，必要时可另行 `alembic downgrade m2a3b4c5d678`。数据损坏时从上述发布前备份恢复 PostgreSQL；MinIO 本轮未修改对象数据。`luyun-demo` 可用 `tailscale serve --https=8443 off` 撤销入口，并停止其 Compose project；普通停用不得加 `-v`。
