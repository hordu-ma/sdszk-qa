...
2026-07-22 当前集成候选：`luyun-int` 应用镜像 `stage2-wp25-orgrbac-20260722-r1`（WP2.5 第二增量：试点组织白名单与最小 RBAC），API/Web 镜像 ID 为 `5ed2cba33a88...` / `b2e874eb6a72...`，迁移升级至 `o4c5d6e7f890 (head)`（新增 `organizations` 表 + `users.organization_id` 列，nullable+回填，向后兼容）。标准 Dockerfile 在线构建成功。真实链路验证：存量 demo_teacher 工作台可用、平台 admin 组织管理成功且教师越权 403、暂停组织成员工作台 403、org A reviewer 对 org B 资源全部 403/404 且复核队列不含跨组织数据集、平台 admin 跨组织可见（记录缺口）、vLLM 未降级、本机与 Tailnet 两个 HTTPS `/healthz` 通过。应用已实际回滚至上一镜像 `stage2-wp25-security-20260720-r1` 后健康（nullable+回填保证向后兼容，无需数据库降级）并恢复当前镜像。发布前备份位于 `/home/pgx/backups/luyun-sizheng/20260722-stage2-wp25-orgrbac-r1-predeploy/`。详细记录见《实施记录附录》§20–§21；收口范围见《[WP2.5 收口记录](../docs/2026-stage2-wp25-closure.md)》。
...
早前增量已修复 MinIO healthcheck：旧配置 `mc ready local` 固定探测 `localhost:9000`，与 Base-Spark 的 `29000/30000` 实际监听不一致；当前检查通过 `MINIO_PORT` 生成目标地址，并在容器内展开已有凭据。回滚当前 WP2.5 组织/RBAC 应用只需将 `luyun-int` 仓库外 env 的 `RELEASE_TAG` 改回 `stage2-wp25-security-20260720-r1` 并重新创建 API/Web；迁移 `o4c5d6e7f890` 仅新增 `organizations` 表与 nullable 且已回填的 `users.organization_id` 列，旧镜像可直接运行，无需数据库降级（必要时可另行 `alembic downgrade n3b4c5d6e789`）。数据损坏时从上述发布前备份恢复 PostgreSQL；MinIO 本轮未修改对象数据。`luyun-demo` 可用 `tailscale serve --https=8443 off` 撤销入口，并停止其 Compose project；普通停用不得加 `-v`。
